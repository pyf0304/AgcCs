...
strSQL = $"Select Top {intPageSize} PageDispMode.* " + 
$"from PageDispMode " + 
$"{strLeftLinkStr} " + 
$"where {strCondition} and PageDispMode.PageDispModeId not in " + 
$"(Select top {intTop_In} PageDispMode.PageDispModeId from PageDispMode " + 
$"{strLeftLinkStr} " +
$" where {strCondition} " + 
...
try
{
 ...
 objPageDispModeEN.Memo = objDT.Rows[0][conPageDispMode.Memo].ToString().Trim(); //说明(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
try
{
 ...
 objPageDispModeEN.Memo = objRow[conPageDispMode.Memo] == DBNull.Value ? null : objRow[conPageDispMode.Memo].ToString().Trim(); //说明(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
Memo = objRow[conPageDispMode.Memo] == DBNull.Value ? null : objRow[conPageDispMode.Memo].ToString().Trim() //说明
};
objPageDispModeEN.ClearUpdateState();//清除修改状态,即清除脏字段信息
return objPageDispModeEN;
}
...
objRow[conPageDispMode.PageDispModeENName] = objPageDispModeEN.PageDispModeENName; //页面显示模式英文名
 ...
 {
objRow[conPageDispMode.Memo] = objPageDispModeEN.Memo; //说明
 }
...
objRow[conPageDispMode.Memo] = oRow[conPageDispMode.Memo].ToString().Trim(); //说明
 objDS.Tables[clsPageDispModeEN._CurrTabName].Rows.Add(objRow);
}
try
{
...
sbSQL.AppendFormat("Update PageDispMode Set ");
 ...
 {
 var strMemo = objPageDispModeEN.Memo.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat("{1} = '{0}',", strMemo, conPageDispMode.Memo); //说明
 }
 else
 {
 sbSQL.AppendFormat("{0} = null,",conPageDispMode.Memo); //说明
 }
 }
