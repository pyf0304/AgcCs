...
strSQL = $"Select Top {intPageSize} CacheMode.* " + 
$"from CacheMode " + 
$"{strLeftLinkStr} " + 
$"where {strCondition} and CacheMode.CacheModeId not in " + 
$"(Select top {intTop_In} CacheMode.CacheModeId from CacheMode " + 
$"{strLeftLinkStr} " +
$" where {strCondition} " + 
...
try
{
 ...
 objCacheModeEN.Memo = objDT.Rows[0][conCacheMode.Memo].ToString().Trim(); //说明(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
try
{
 ...
 objCacheModeEN.Memo = objRow[conCacheMode.Memo] == DBNull.Value ? null : objRow[conCacheMode.Memo].ToString().Trim(); //说明(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
Memo = objRow[conCacheMode.Memo] == DBNull.Value ? null : objRow[conCacheMode.Memo].ToString().Trim() //说明
};
objCacheModeEN.ClearUpdateState();//清除修改状态,即清除脏字段信息
return objCacheModeEN;
}
...
objRow[conCacheMode.InUse] = objCacheModeEN.InUse; //是否在用
 ...
 {
objRow[conCacheMode.Memo] = objCacheModeEN.Memo; //说明
 }
...
objRow[conCacheMode.Memo] = oRow[conCacheMode.Memo].ToString().Trim(); //说明
 objDS.Tables[clsCacheModeEN._CurrTabName].Rows.Add(objRow);
}
try
{
...
sbSQL.AppendFormat("Update CacheMode Set ");
 ...
 {
 var strMemo = objCacheModeEN.Memo.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat("{1} = '{0}',", strMemo, conCacheMode.Memo); //说明
 }
 else
 {
 sbSQL.AppendFormat("{0} = null,",conCacheMode.Memo); //说明
 }
 }
