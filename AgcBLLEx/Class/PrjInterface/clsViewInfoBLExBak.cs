...
{
    public class ASPDropDownListComparerBak : IEqualityComparer<ASPDropDownListEx>
    {
    ...
    public static class clsViewInfoBLEx_StaticBak
    {
    ...
    public partial class clsViewInfoBLExBak : clsViewInfoBL
    {
        public static string strPrjIdCache_Init = "";
        public clsViewInfoBLExBak()
        {
        }
        public clsViewInfoBLExBak(string strViewId)
        {
        ...
        /// <summary>
        /// 复制界面（从源工程复制界面到目标工程）
        /// 参考 clsPrjTabBLEx.CopyPrjTab 的实现逻辑
        /// </summary>
        /// <param name="strTarPrjId">目标工程ID</param>
        /// <param name="strSouViewId">源界面ID</param>
        /// <param name="strUserId">操作用户ID</param>
        /// <returns>返回新界面ID，失败返回空字符串</returns>
        public static string CopyViewInfo(string strTarPrjId, string strSouViewId, string strUserId)
        {
            string strNewViewId = "";
            ...
            try
            {
                ...
                {
                    throw new Exception(string.Format("源界面ID:[{0}]不存在，无法复制", strSouViewId));
                }

                string strSouPrjId = objSouViewInfo.PrjId;

                // 2、检查目标工程中是否存在同名界面
                string strCondition = string.Format("PrjId = '{0}' and ViewName = '{1}'",
                    strTarPrjId, objSouViewInfo.ViewName);

                if (clsViewInfoBL.IsExistRecord(strCondition))
                {
                    strNewViewId = clsViewInfoBL.GetFirstID_S(strCondition);
                    string strMsg = string.Format("目标工程中已存在同名界面:[{0}]，返回已存在界面ID:[{1}]",
                        objSouViewInfo.ViewName, strNewViewId);
                    clsPubVar4BLEx.objLog4Error.WriteDebugLog(strMsg);
                    return strNewViewId;
                }

                // 3、复制主表到目标工程
                string strNewMainTabId = "";
                if (!string.IsNullOrEmpty(objSouViewInfo.MainTabId))
                {
                    clsPrjTabEN objMainTab = clsPrjTabBL.GetObjByTabIdCache(objSouViewInfo.MainTabId, strSouPrjId);
                    if (objMainTab != null)
                    {
                        string strCondTabId = string.Format("PrjId = '{0}' and TabName = '{1}'",
                            strTarPrjId, objMainTab.TabName);

                        if (clsPrjTabBL.IsExistRecord(strCondTabId))
                        {
                            strNewMainTabId = clsPrjTabBL.GetFirstID_S(strCondTabId);
                        }
                        else
                        {
                            strNewMainTabId = clsPrjTabBLEx.CopyPrjTab(strTarPrjId, objSouViewInfo.MainTabId, strUserId);
                        }
                    }
                }

                // 复制InRelaTabId
                string strNewInRelaTabId = "";
                if (!string.IsNullOrEmpty(objSouViewInfo.InRelaTabId))
                {
                    clsPrjTabEN objInRelaTab = clsPrjTabBL.GetObjByTabIdCache(objSouViewInfo.InRelaTabId, strSouPrjId);
                    if (objInRelaTab != null)
                    {
                        string strCondTabId = string.Format("PrjId = '{0}' and TabName = '{1}'",
                            strTarPrjId, objInRelaTab.TabName);

                        if (clsPrjTabBL.IsExistRecord(strCondTabId))
                        {
                            strNewInRelaTabId = clsPrjTabBL.GetFirstID_S(strCondTabId);
                        }
                        else
                        {
                            strNewInRelaTabId = clsPrjTabBLEx.CopyPrjTab(strTarPrjId, objSouViewInfo.InRelaTabId, strUserId);
                        }
                    }
                }

                // 复制OutRelaTabId
                string strNewOutRelaTabId = "";
                if (!string.IsNullOrEmpty(objSouViewInfo.OutRelaTabId))
                {
                    clsPrjTabEN objOutRelaTab = clsPrjTabBL.GetObjByTabIdCache(objSouViewInfo.OutRelaTabId, strSouPrjId);
                    if (objOutRelaTab != null)
                    {
                        string strCondTabId = string.Format("PrjId = '{0}' and TabName = '{1}'",
                            strTarPrjId, objOutRelaTab.TabName);

                        if (clsPrjTabBL.IsExistRecord(strCondTabId))
                        {
                            strNewOutRelaTabId = clsPrjTabBL.GetFirstID_S(strCondTabId);
                        }
                        else
                        {
                            strNewOutRelaTabId = clsPrjTabBLEx.CopyPrjTab(strTarPrjId, objSouViewInfo.OutRelaTabId, strUserId);
                        }
                    }
                }

                // 复制DetailTabId
                string strNewDetailTabId = "";
                if (!string.IsNullOrEmpty(objSouViewInfo.DetailTabId))
                {
                    clsPrjTabEN objDetailTab = clsPrjTabBL.GetObjByTabIdCache(objSouViewInfo.DetailTabId, strSouPrjId);
                    if (objDetailTab != null)
                    {
                        string strCondTabId = string.Format("PrjId = '{0}' and TabName = '{1}'",
                            strTarPrjId, objDetailTab.TabName);

                        if (clsPrjTabBL.IsExistRecord(strCondTabId))
                        {
                            strNewDetailTabId = clsPrjTabBL.GetFirstID_S(strCondTabId);
                        }
                        else
                        {
                            strNewDetailTabId = clsPrjTabBLEx.CopyPrjTab(strTarPrjId, objSouViewInfo.DetailTabId, strUserId);
                        }
                    }
                }

                // 4、创建新界面对象并复制属性
                strNewViewId = clsGeneralTab.GetMaxStrId("ViewInfo", "ViewId", 8, strTarPrjId);
                clsViewInfoEN objNewViewInfo = new clsViewInfoEN(strNewViewId);

                objNewViewInfo.ViewId = strNewViewId;
                objNewViewInfo.ViewName = objSouViewInfo.ViewName;
                objNewViewInfo.ViewCnName = objSouViewInfo.ViewCnName;
                objNewViewInfo.PrjId = strTarPrjId;

                objNewViewInfo.MainTabId = strNewMainTabId;
                objNewViewInfo.InRelaTabId = strNewInRelaTabId;
                objNewViewInfo.OutRelaTabId = strNewOutRelaTabId;
                objNewViewInfo.DetailTabId = strNewDetailTabId;
                ...
                foreach (clsViewRegionRelaEN objSouRegionRela in arrSouRegionRelaLst)
                {
                    ...
                    string strNewRegionId = "";

                    string strCheckRegionCond = string.Format("PrjId = '{0}' and ClsName = '{1}'",
                        strTarPrjId, objSouRegion.ClsName);

                    if (clsViewRegionBL.IsExistRecord(strCheckRegionCond))
                    {
                        strNewRegionId = clsViewRegionBL.GetFirstID_S(strCheckRegionCond);
                        string strMsg = string.Format("目标工程中已存在同名区域:[{0}]，使用现有区域ID:[{1}]",
                            objSouRegion.ClsName, strNewRegionId);
                        clsPubVar4BLEx.objLog4Error.WriteDebugLog(strMsg);
                    }
                    else
                    {
                        strNewRegionId = clsGeneralTab.GetMaxStrId("ViewRegion", "RegionId", 8, strTarPrjId);
                        ...
                        objNewRegion.PrjId = strTarPrjId;
                        objNewRegion.UpdDate = clsDateTime.getTodayDateTimeStr(1);
                        objNewRegion.UpdUser = strUserId;

                        if (!string.IsNullOrEmpty(objSouRegion.TabId))
                        {
                            clsPrjTabEN objRegionTab = clsPrjTabBL.GetObjByTabIdCache(objSouRegion.TabId, strSouPrjId);
                            if (objRegionTab != null)
                            {
                                string strCondRegionTabId = string.Format("PrjId = '{0}' and TabName = '{1}'",
                                    strTarPrjId, objRegionTab.TabName);

                                if (clsPrjTabBL.IsExistRecord(strCondRegionTabId))
                                {
                                    objNewRegion.TabId = clsPrjTabBL.GetFirstID_S(strCondRegionTabId);
                                }
                                else
                                {
                                    objNewRegion.TabId = clsPrjTabBLEx.CopyPrjTab(strTarPrjId, objSouRegion.TabId, strUserId);
                                }
                            }
                        }
                        ...
                        {
                            string strErrMsg = string.Format("添加新区域记录失败，区域类型:[{0}]", objSouRegion.RegionTypeId);
                            clsPubVar4BLEx.objLog4Error.WriteDebugLog(strErrMsg);
                            continue;
                        }

                        CopyRegionFields(objSouRegion.RegionId, strNewRegionId, objSouRegion.RegionTypeId,
                            strSouPrjId, strTarPrjId, strUserId);
                    }

                    string strCheckRelaCondition = string.Format("RegionId = '{0}' and ViewId = '{1}'",
                        strNewRegionId, strNewViewId);

                    if (!clsViewRegionRelaBL.IsExistRecord(strCheckRelaCondition))
                    {
                        clsViewRegionRelaEN objNewRegionRela = new clsViewRegionRelaEN();
                        objNewRegionRela.RegionId = strNewRegionId;
                        objNewRegionRela.ViewId = strNewViewId;
                        objNewRegionRela.PrjId = strTarPrjId;
                        objNewRegionRela.InUse = objSouRegionRela.InUse;
                        objNewRegionRela.IsDisp = objSouRegionRela.IsDisp;
                        objNewRegionRela.UpdDate = clsDateTime.getTodayDateTimeStr(1);
                        objNewRegionRela.UpdUser = strUserId;
                        clsViewRegionRelaBL.AddNewRecordBySql2(objNewRegionRela);
                    }
                }
                ...
                string strSuccessMsg = string.Format("成功复制界面:[{0}]，新界面ID:[{1}]",
                    objSouViewInfo.ViewName, strNewViewId);
                clsPubVar4BLEx.objLog4Error.WriteDebugLog(strSuccessMsg);

                return strNewViewId;
            }
            catch (Exception objException)
            {
                string strMsg = string.Format("复制界面失败，源界面ID:[{0}]，错误:{1}.(in {2})",
                    strSouViewId, objException.Message, clsStackTrace.GetCurrClassFunction());
                clsPubVar4BLEx.objLog4Error.WriteDebugLog(strMsg);
                throw new Exception(strMsg);
            }
        }
