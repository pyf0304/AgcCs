...
namespace AGC.Webform
{
    ...
    /// </summary>
    public partial class wfmFieldTab_QUDI : CommWebPageBase
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)	///如果本界面是初次被调用，而不是单击按钮事件所调用
            {
                ...
                clsDropDownList.BindDdl_TrueAndFalse(ddlIsNeedTransCode);
                ...
                //clsvFieldTab_SimBL.BindDdl_FldId(ddlFldId, clsPubVar.CurrSelPrjId);
        ...
        private string CombineFieldTabCondition()
        {
            ...
            //if (this.ddlIsNeedTransCode.SelectedIndex == 1)
            //{
            //    strWhereCond += string.Format(" and {0}='1'", conFieldTab.IsNeedTransCode);
            //}
            //else if (this.ddlIsNeedTransCode.SelectedIndex == 2)
            //{
        ...
        protected void btnQuery_Click(object sender, System.EventArgs e)
        {
            ...
            this.wucvFieldTab4Gv1.vsPageIndex = 1;
            //把表记录的内容显示在DATAGRID中
            BindGv_vFieldTab();
        }
...
var                         objFieldTab4CodeConv = new clsFieldTab4CodeConvEN(objFieldTabEN.FldId);  //初始化新对象
                        ...
                        if (objFieldTab4CodeConv.AddRecordEx() == false)
                        {
                            strMsg = "添加不成功!";
                            clsCommonJsFunc.Alert(this, strMsg);
                            lblMsg.Text = "添加记录不成功!";
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                //clsFldObjTabBLEx.RefreshUpdDate4ReleObjects(objFieldTabEN.FldId, clsCommonSession.UserId);
        ...
        protected void lbInsNewRec_Click(object sender, System.EventArgs e)
        {
            ...
            if (clsFieldTabBL.IsExistRecord(strCondition))	//判断是否有相同的关键字
            {
                ...
                //clsFldObjTabBLEx.CreateFldObjRelation(ObjId, strFldId);
                BindGv_vFieldTab();
                return;
            }
        ...
        {
            List<string> lstFldID = wucvFieldTab4Gv1.GetAllCheckedKeysFromGv();
        ...
        {
            string strFldID = wucvFieldTab4Gv1.GetFirstCheckedKeyFromGv();
        ...
        /// <param name = "sender"></param>
        /// <param name = "e"></param>
        protected void wucvFieldTab4Gv1_GvDeleteClick(object sender, clsEventArgs4Delete e)
        {
        ...
        /// <param name = "sender"></param>
        /// <param name = "e"></param>
        protected void wucvFieldTab4Gv1_GvUpdateClick(object sender, clsEventArgs4Update e)
        {
            string strFldID = e.KeyId;
        ...
        /// </summary>
        protected void BindGv_vFieldTab()
        {
            //操作步骤:(共5步)
            //	1、组合界面条件串；
            ...
            try
            {
                ...
                //	2、调用控件中的绑定GridView函数；
                wucvFieldTab4Gv1.BindGv_FieldTab(strWhereCond);
                //	3、设置Gv控件的某些列不可见；
                wucvFieldTab4Gv1.SetGvVisibility(convFieldTab.UpdDate, false);
                wucvFieldTab4Gv1.SetGvVisibility(convFieldTab.FldCnName, false);

                wucvFieldTab4Gv1.SetGvVisibility(convFieldTab.IsChecked, false);
                wucvFieldTab4Gv1.SetGvVisibility(convFieldTab.IsArchive, false);
                wucvFieldTab4Gv1.SetGvVisibility(convFieldTab.DefaultValue, false);
                wucvFieldTab4Gv1.SetGvVisibility(convFieldTab.IsOnlyOne, false);
        ...
        protected void lbSetIsNeedTrans_Click(object sender, EventArgs e)
        {
            List<string> lstFldID = wucvFieldTab4Gv1.GetAllCheckedKeysFromGv();
            if (lstFldID == null) return;
            lblMsgList.Text = "";
            try
            {
                ...
                {
                    clsFieldTabEN objFieldTabEN = clsFieldTabBL.GetObjByFldId(strFldId);
                    //objFieldTabEN.IsNeedTransCode = !objFieldTabEN.IsNeedTransCode;
                    objFieldTabEN.UpdateRecordEx();
                    clsPrjTabFldBLEx.RefreshUpdDate4ReleTab(objFieldTabEN.FldId, clsCommonSession.UserId);
                }
            }
            ...
            {
                clsCommonSession.seErrMessage = "设置是否转换不成功!\r\n" + objException.Message;
        ...
        protected void lbSetHomologousFld_Click(object sender, EventArgs e)
        {
            ...
            if (lstFldID == null) 
            {
                lblMsgList.Text = "没有选择记录，请选择有效的字段记录!";
                clsCommonJsFunc.Alert(this, lblMsgList.Text);
                return;
            }
            try
            {
                var strHomologousFldId = ddlFldId.SelectedValue;
                foreach (string strFldId in lstFldID)
                {
                    var objFieldTab = clsFieldTabBL.GetObjByFldId(strFldId);
                    objFieldTab.HomologousFldId = strHomologousFldId;
                    objFieldTab.UpdDate = clsDateTime.getTodayDateTimeStr(0);
                    objFieldTab.UpdUser = clsCommonSession.UserId;
                    objFieldTab.UpdateRecordEx();
                }
                BindGv_vFieldTab();
            }
            catch (Exception objException)
            {
              var  strMsg = "设置同源字段不成功!" + objException.Message;
                clsCommonJsFunc.Alert(this, strMsg);
                lblMsg.Text = strMsg;
                return;
            }
    ...
    public partial class wfmFieldTab_QUDI : CommWebPageBase
    {
        ...
        private void CopyField()
        {
            ...
            try
            {
                ...
                {
                    clsFieldTabEN objFieldTabEN = clsFieldTabBL.GetObjByFldId(strFldId);
                    objFieldTabEN.SetFldId(clsFieldTabBL.GetMaxStrIdByPrefix_S(objFieldTabEN.PrjId))
                        .SetFldName("C_" + objFieldTabEN.FldName)
