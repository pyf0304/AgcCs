...
namespace AutoGCLib
{
    ...
    /// 用于定义查询区和功能区的按钮
    /// </summary>
    partial class Vue_ViewScriptCommands_TS4TypeScriptBak : Vue_ViewScriptCS_TS4TypeScript
    {
        private readonly RenderService _renderService;

        public Vue_ViewScriptCommands_TS4TypeScriptBak(string strViewId, string strPrjDataBaseId, string strPrjId)
        ...
        private void AddFeatureCommands(AiCommandTemplateModel model)
        {
            ...
            foreach (var feature in setFieldFeatures)
            {
                try
                {
                    ...
                    {
                        buttonText = "设置" + (feature.ObjFieldTabENEx?.Caption ?? fldName);
                    }

                    // 🔥 确定辅助控件类型
                    string auxControlType = "select";  // 默认为普通下拉框
                    string auxControlOptionsKey = null;
                    string csType = objFieldTab.CsType();
                    
                    if (csType.ToLower() == "bool" || csType.ToLower() == "boolean")
                    {
                        auxControlType = "select4Bool";  // 布尔类型下拉框
                    }
                    else
                    {
                        // 🔥 推断关联表名（如 UseStateId → UseState）
                        string relatedTableName = fldName.EndsWith("Id") 
                            ? fldName.Substring(0, fldName.Length - 2) 
                            : fldName;
                        
                        // 🔥 设置选项键（用于从 VueShare 获取数据源）
                        auxControlOptionsKey = relatedTableName + "_Static";
                    }
                    ...
                    model.Commands.Add(new AiCommand
                    {
                        ...
                        BtnClass = "btn btn-outline-info btn-sm text-nowrap",
                        NeedAuxControl = true,
                        AuxControlId = $"ddl{fldName}_SetFldValue",       // 🔥 下拉框ID
                        AuxControlType = auxControlType,                   // 🔥 控件类型
                        AuxControlOptionsKey = auxControlOptionsKey,       // 🔥 选项键
                        FieldName = fldName,                               // 🔥 字段名
                        FieldNameCamel = ToCamelCase(fldName)             // 🔥 驼峰字段名
                    });
                }
                catch (Exception ex)
                {
                    // 记录错误但继续处理其他功能
                    Console.WriteLine($"处理设置字段值功能时出错: {ex.Message}");
                }
            }
        }
