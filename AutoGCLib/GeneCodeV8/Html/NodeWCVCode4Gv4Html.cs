...
namespace AutoGCLib
{
    ...
    /// </summary>
    public class NodeWCVCode4Gv4Html : clsGeneCodeBase4View
    {
        
        
        #region 构造函数
        public NodeWCVCode4Gv4Html()
        {
            // 
            // TODO: 在此处添加构造函数逻辑
            //
        ...
        public NodeWCVCode4Gv4Html(string strViewId)
       : base(strViewId, "", "")
        {
            // 
            // TODO: 在此处添加构造函数逻辑
        ...
        public NodeWCVCode4Gv4Html( string strViewId, string strPrjDataBaseId, string strPrjId)
        ...
        public string GenGridView4WucGv(clsViewInfoENEx objViewInfoENEx)
        {
            ...
            try
            {
                ...
                strCodeForCs.AppendFormat("\r\n" + "OnRowCommand = \"gv{0}_RowCommand\"",
                  objViewInfoENEx.TabName);
                if (objViewInfoENEx.arrFeatureRegionFlds.Exists(x => x.RegionId == objViewInfoENEx.objViewRegion_List.RegionId && x.FeatureId == enumPrjFeature.DelRecord_Gv_0175))
                {
                    strCodeForCs.AppendFormat("\r\n" + "OnRowDeleting = \"gv{0}_RowDeleting\" ",
                      objViewInfoENEx.TabName);
                }
                if (objViewInfoENEx.arrFeatureRegionFlds.Exists(x => x.RegionId == objViewInfoENEx.objViewRegion_List.RegionId 
                        && x.FeatureId == enumPrjFeature.UpdateRecord_Gv_0174))
                {
                    strCodeForCs.AppendFormat("\r\n" + "OnRowUpdating = \"gv{0}_RowUpdating\" ",
                      objViewInfoENEx.TabName);
                }
                if (objViewInfoENEx.arrFeatureRegionFlds.Exists(x => x.RegionId == objViewInfoENEx.objViewRegion_List.RegionId && x.FeatureId == enumPrjFeature.DetailRecord_Gv_0181))
                {
                    strCodeForCs.AppendFormat("\r\n" + "OnRowDetailing = \"gv{0}_RowDetailing\" ",
                      objViewInfoENEx.TabName);
                }
                if (objViewInfoENEx.arrFeatureRegionFlds.Exists(x => x.RegionId == objViewInfoENEx.objViewRegion_List.RegionId && x.FeatureId == enumPrjFeature.SelectRecord_Gv_0182))
                {
...
//                                     strCodeForCs.Append("\r\n" + "<asp:LinkButton id = \"lbSelAll\" CommandName = \"lbSelAll\" runat = \"server\" CssClass = \"btn btn-link btn-sm text-nowrap\" Text = \"全选\"></asp:LinkButton>");
        ...
        public override string A_GeneFuncCode(clsvFunction4GeneCodeEN objvFunction4GeneCodeEN, ref clsFunction4CodeEN Re_objFunction4Code)
        {
            ...
            {
                string strCode = "";
                Type t = typeof(NodeWCVCode4Gv4Html);
