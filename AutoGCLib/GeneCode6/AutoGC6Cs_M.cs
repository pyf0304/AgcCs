...
namespace AutoGCLib
{
    ...
    /// </summary>
    class AutoGC6Cs_M : clsGeneCodeBase4Tab
    {
        ...
        public AutoGC6Cs_M(string strTabId, string strPrjDataBaseId, string strPrjId)
        ...
        public string A_GenSQLCode4CreateTab(string strPrjDataBaseId)
        {
            ...
            if (objPrjTabENEx.TabFldNum() == 0)
            {
                clsPrjTabBL.ReFreshCache(objPrjTabENEx.PrjId);
                strResult = string.Format("当前表:[{0}]的字段数为0,无法建表,可以再次尝试!({1})",
                           objPrjTabENEx.TabName, clsStackTrace.GetCurrClassFunction());
                throw new clsDbObjException(strResult);
            }
            int intKeyFldNum = objPrjTabENEx.KeyFldNum();
            if (intKeyFldNum == 0)
            {
                clsPrjTabBL.ReFreshCache(objPrjTabENEx.PrjId);
                strResult = string.Format("当前表:[{0}]没有关键字,无法建表,可以再次尝试!({1})",
                 objPrjTabENEx.TabName, clsStackTrace.GetCurrClassFunction());
                throw new clsDbObjException(strResult);
            }
            ...
            List<string> lstPrimaryKey = new List<string>();       //, strPrimaryKey0;
            ...
            if (intKeyFldNum == 1)
            {
                ...
                {
                    if (objField.IsForExtendClass == true) continue;
            ...
            else
            {
                foreach (clsPrjTabFldENEx objField in objPrjTabENEx.arrFldSetAll)
                {
                    ...
                    else if (objField.FieldTypeId == enumFieldType.KeyField_02)
                    {
                        if (objField.IsTabNullable == true)
                        {
                            string strMsg = string.Format("关键字段:[{0}]不能为null.", objField.FldName);
                            throw new Exception(strMsg);
                        }
                        lstPrimaryKey.Add(objField.FldName);
                    }
                ...
                string strConstraintName = string.Join("_", lstPrimaryKey);
                string strKeyLst = string.Join(",", lstPrimaryKey);

                strCreateTabCode.AppendFormat("CREATE table [{2}].[{0}] ({1},\r\r\n CONSTRAINT {3} PRIMARY KEY ({4}) )\r\r\n", 
                    objPrjTabENEx.TabName, strTabBodyCode, strDatabaseOwner, strConstraintName, strKeyLst);
                //strCreateTabCode.Append("ON [PRIMARY] ");
            }
