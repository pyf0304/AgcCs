...
clsSqlDataSynWApi.ReFreshCache();
return bolResult;
}
...
from objSqlDataSynEN in arrSqlDataSynObjLstCache
where objSqlDataSynEN.TabId == strTabId 
select objSqlDataSynEN;
if (arrSqlDataSynObjLst_Sel.Count() == 0)
{
   clsSqlDataSynEN obj = clsSqlDataSynWApi.GetObjByTabId(strTabId);
   if (obj != null)
 {
...
public static IEnumerable<clsSqlDataSynEN> GetObjLstByTabIdLstCache(List<string> arrTabId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsSqlDataSynEN._CurrTabName);
List<clsSqlDataSynEN> arrSqlDataSynObjLstCache = GetObjLstCache();
IEnumerable <clsSqlDataSynEN> arrSqlDataSynObjLst_Sel =
from objSqlDataSynEN in arrSqlDataSynObjLstCache
where arrTabId.Contains(objSqlDataSynEN.TabId)
select objSqlDataSynEN;
...
if (clsSqlDataSynEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsSqlDataSynEN._AttributeName)
{
...
var strKey = clsSqlDataSynEN._CurrTabName;
List<clsSqlDataSynEN> arrSqlDataSynObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
