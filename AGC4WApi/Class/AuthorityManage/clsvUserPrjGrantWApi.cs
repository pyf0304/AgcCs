...
from objvUserPrjGrantEN in arrvUserPrjGrantObjLstCache
where objvUserPrjGrantEN.mId == lngmId 
select objvUserPrjGrantEN;
if (arrvUserPrjGrantObjLst_Sel.Count() == 0)
{
   clsvUserPrjGrantEN obj = clsvUserPrjGrantWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvUserPrjGrantEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvUserPrjGrantEN._CurrTabName);
List<clsvUserPrjGrantEN> arrvUserPrjGrantObjLstCache = GetObjLstCache();
IEnumerable <clsvUserPrjGrantEN> arrvUserPrjGrantObjLst_Sel =
from objvUserPrjGrantEN in arrvUserPrjGrantObjLstCache
where arrMId.Contains(objvUserPrjGrantEN.mId)
select objvUserPrjGrantEN;
...
if (clsvUserPrjGrantEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvUserPrjGrantEN._AttributeName)
{
...
var strKey = clsvUserPrjGrantEN._CurrTabName;
List<clsvUserPrjGrantEN> arrvUserPrjGrantObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
