...
from objvWebSrvClassEN in arrvWebSrvClassObjLstCache
where objvWebSrvClassEN.WebSrvClassId == strWebSrvClassId 
select objvWebSrvClassEN;
if (arrvWebSrvClassObjLst_Sel.Count() == 0)
{
   clsvWebSrvClassEN obj = clsvWebSrvClassWApi.GetObjByWebSrvClassId(strWebSrvClassId);
   if (obj != null)
 {
...
public static IEnumerable<clsvWebSrvClassEN> GetObjLstByWebSrvClassIdLstCache(List<string> arrWebSrvClassId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsvWebSrvClassEN._CurrTabName, strPrjId);
List<clsvWebSrvClassEN> arrvWebSrvClassObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsvWebSrvClassEN> arrvWebSrvClassObjLst_Sel =
from objvWebSrvClassEN in arrvWebSrvClassObjLstCache
where arrWebSrvClassId.Contains(objvWebSrvClassEN.WebSrvClassId)
select objvWebSrvClassEN;
...
if (clsvWebSrvClassEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvWebSrvClassEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvWebSrvClassWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvWebSrvClassWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvWebSrvClassWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvWebSrvClassWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsvWebSrvClassEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsvWebSrvClassEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",convWebSrvClass.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsvWebSrvClassEN._CurrTabName, strPrjId);
List<clsvWebSrvClassEN> arrvWebSrvClassObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrvWebSrvClassObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsvWebSrvClassEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsvWebSrvClassEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvWebSrvClassWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvWebSrvClassWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsvWebSrvClassEN._CurrTabName, strPrjId);
List<clsvWebSrvClassEN> arrvWebSrvClassObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrvWebSrvClassObjLstCache = CacheHelper.Get<List<clsvWebSrvClassEN>>(strKey);
}
else
{
