...
from objvCodeTypeEN in arrvCodeTypeObjLstCache
where objvCodeTypeEN.CodeTypeId == strCodeTypeId 
select objvCodeTypeEN;
if (arrvCodeTypeObjLst_Sel.Count() == 0)
{
   clsvCodeTypeEN obj = clsvCodeTypeWApi.GetObjByCodeTypeId(strCodeTypeId);
   if (obj != null)
 {
...
public static IEnumerable<clsvCodeTypeEN> GetObjLstByCodeTypeIdLstCache(List<string> arrCodeTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvCodeTypeEN._CurrTabName);
List<clsvCodeTypeEN> arrvCodeTypeObjLstCache = GetObjLstCache();
IEnumerable <clsvCodeTypeEN> arrvCodeTypeObjLst_Sel =
from objvCodeTypeEN in arrvCodeTypeObjLstCache
where arrCodeTypeId.Contains(objvCodeTypeEN.CodeTypeId)
select objvCodeTypeEN;
...
if (clsvCodeTypeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvCodeTypeEN._AttributeName)
{
...
var strKey = clsvCodeTypeEN._CurrTabName;
List<clsvCodeTypeEN> arrvCodeTypeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
