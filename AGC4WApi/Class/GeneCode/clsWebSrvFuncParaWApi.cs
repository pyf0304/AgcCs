...
clsWebSrvFuncParaWApi.ReFreshCache();
return bolResult;
}
...
{
if (strWebSrvFuncParaId == "") return null;
...
{
if (string.IsNullOrEmpty(strWebSrvFuncParaId) == true) return null;
//初始化列表缓存
...
from objWebSrvFuncParaEN in arrWebSrvFuncParaObjLstCache
where objWebSrvFuncParaEN.WebSrvFuncParaId == strWebSrvFuncParaId 
select objWebSrvFuncParaEN;
if (arrWebSrvFuncParaObjLst_Sel.Count() == 0)
{
   clsWebSrvFuncParaEN obj = clsWebSrvFuncParaWApi.GetObjByWebSrvFuncParaId(strWebSrvFuncParaId);
   if (obj != null)
 {
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrWebSrvFuncParaId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsWebSrvFuncParaEN> GetObjLstByWebSrvFuncParaIdLstCache(List<string> arrWebSrvFuncParaId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsWebSrvFuncParaEN._CurrTabName);
List<clsWebSrvFuncParaEN> arrWebSrvFuncParaObjLstCache = GetObjLstCache();
IEnumerable <clsWebSrvFuncParaEN> arrWebSrvFuncParaObjLst_Sel =
from objWebSrvFuncParaEN in arrWebSrvFuncParaObjLstCache
where arrWebSrvFuncParaId.Contains(objWebSrvFuncParaEN.WebSrvFuncParaId)
select objWebSrvFuncParaEN;
...
if (clsWebSrvFuncParaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsWebSrvFuncParaEN._AttributeName)
{
...
var strKey = clsWebSrvFuncParaEN._CurrTabName;
List<clsWebSrvFuncParaEN> arrWebSrvFuncParaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
