...
/// 静态类
/// </summary>
public static class  clsViewDesignWApi_Static
{
 ...
 /// <param name = "strViewDesignId">界面设计Id</param>
...
return objViewDesignEN;
 ...
 /// </summary>
 /// <param name = "objViewDesignEN">需要设置字段值的实体对象</param>
 /// <param name = "strViewDesignName">界面设计名</param>
 /// <param name = "strComparisonOp">比较运算符,如果有值,可用于组织条件串</param>
 /// <returns>返回对象,可以继续连写</returns>
...
try
{
objViewDesignEN.sfUpdFldSetStr = objViewDesignEN.getsfUpdFldSetStr();
clsViewDesignWApi.CheckPropertyNew(objViewDesignEN); 
bool bolResult = clsViewDesignWApi.UpdateRecord(objViewDesignEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsViewDesignWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsViewDesignEN objViewDesignEN)
{
//检测记录是否存在
...
public static bool AddNewRecord(this clsViewDesignEN objViewDesignEN)
{
try
{
 if (string.IsNullOrEmpty(objViewDesignEN.ViewDesignId) == true || clsViewDesignWApi.IsExist(objViewDesignEN.ViewDesignId) == true)
 {
     objViewDesignEN.ViewDesignId = clsViewDesignWApi.GetMaxStrId();
 }
clsViewDesignWApi.CheckPropertyNew(objViewDesignEN); 
bool bolResult = clsViewDesignWApi.AddNewRecord(objViewDesignEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsViewDesignWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objViewDesignEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsViewDesignEN objViewDesignEN)
{
try
{
clsViewDesignWApi.CheckPropertyNew(objViewDesignEN); 
string strViewDesignId = clsViewDesignWApi.AddNewRecordWithMaxId(objViewDesignEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsViewDesignWApi.ReFreshCache();
...
{
clsViewDesignWApi.CheckPropertyNew(objViewDesignEN); 
bool bolResult = clsViewDesignWApi.UpdateWithCondition(objViewDesignEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsViewDesignWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsViewDesignWApi
{
private static readonly string mstrApiControllerName = "ViewDesignApi";
...
List<clsViewDesignEN> arrObjLst = GetObjLst("1=1");
objDDL.DataValueField="ViewDesignId";
objDDL.DataTextField="ViewDesignName";
...
List<clsViewDesignEN> arrObjLst = clsViewDesignWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
if (!Object.Equals(null, objViewDesignEN.ViewDesignId) && GetStrLen(objViewDesignEN.ViewDesignId) > 4)
{
 throw new Exception("字段[界面设计Id]的长度不能超过4!");
}
if (!Object.Equals(null, objViewDesignEN.ViewDesignName) && GetStrLen(objViewDesignEN.ViewDesignName) > 30)
{
 throw new Exception("字段[界面设计名]的长度不能超过30!");
}
if (!Object.Equals(null, objViewDesignEN.Memo) && GetStrLen(objViewDesignEN.Memo) > 1000)
{
...
{
if (strViewDesignId == "") return null;
string strAction = "GetObjByViewDesignId";
clsViewDesignEN objViewDesignEN;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strViewDesignId"] = strViewDesignId,
};
try
{
...
objViewDesignEN = JsonConvert.DeserializeObject<clsViewDesignEN>(strJson);
return objViewDesignEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsViewDesignEN GetFirstObj(string strWhereCond)
{
...
{
if (string.IsNullOrEmpty(strViewDesignId) == true) return null;
//初始化列表缓存
...
from objViewDesignEN in arrViewDesignObjLstCache
where objViewDesignEN.ViewDesignId == strViewDesignId 
select objViewDesignEN;
if (arrViewDesignObjLst_Sel.Count() == 0)
{
   clsViewDesignEN obj = clsViewDesignWApi.GetObjByViewDesignId(strViewDesignId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsViewDesignEN> arrViewDesignObjLstCache = GetObjLstCache();
IEnumerable <clsViewDesignEN> arrViewDesignObjLst_Sel1 =
from objViewDesignEN in arrViewDesignObjLstCache
where objViewDesignEN.ViewDesignId == strViewDesignId 
select objViewDesignEN;
List <clsViewDesignEN> arrViewDesignObjLst_Sel = new List<clsViewDesignEN>();
foreach (clsViewDesignEN obj in arrViewDesignObjLst_Sel1)
{
arrViewDesignObjLst_Sel.Add(obj);
}
if (arrViewDesignObjLst_Sel.Count > 0)
{
return arrViewDesignObjLst_Sel[0].ViewDesignName;
}
string strErrMsgForGetObjById = string.Format("在ViewDesign对象缓存列表中,找不到记录[ViewDesignId = {0}](函数:{1})", strViewDesignId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsViewDesignBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在ViewDesign对象缓存列表中,找不到记录的相关名称[ViewDesignId = {0}](函数:{1})", strViewDesignId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsViewDesignBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
{
 List<clsViewDesignEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsViewDesignEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrViewDesignId">关键字列表</param>
 /// <returns>返回对象列表</returns>
public static List<clsViewDesignEN> GetObjLstByViewDesignIdLst(List<string> arrViewDesignId)
{
 List<clsViewDesignEN> arrObjLst; 
string strAction = "GetObjLstByViewDesignIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrViewDesignId);
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrViewDesignId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsViewDesignEN> GetObjLstByViewDesignIdLstCache(List<string> arrViewDesignId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsViewDesignEN._CurrTabName);
List<clsViewDesignEN> arrViewDesignObjLstCache = GetObjLstCache();
IEnumerable <clsViewDesignEN> arrViewDesignObjLst_Sel =
from objViewDesignEN in arrViewDesignObjLstCache
where arrViewDesignId.Contains(objViewDesignEN.ViewDesignId)
select objViewDesignEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsViewDesignEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsViewDesignEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsViewDesignEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsViewDesignEN> arrObjLst; 
...
try
{
 clsViewDesignEN objViewDesignEN = clsViewDesignWApi.GetObjByViewDesignId(strViewDesignId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strViewDesignId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelViewDesigns(List<string> arrViewDesignId)
{
string strAction = "DelViewDesigns";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrViewDesignId);
...
{
string strAction = "DelViewDesignsByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsViewDesignEN objViewDesignEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsViewDesignEN>(objViewDesignEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsViewDesignEN objViewDesignEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsViewDesignWApi.ReFreshCache();
var strViewDesignId = (string)jobjReturn0["returnStr"];
return strViewDesignId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsViewDesignEN objViewDesignEN)
{
if (string.IsNullOrEmpty(objViewDesignEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objViewDesignEN.ViewDesignId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsViewDesignEN objViewDesignEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objViewDesignEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsViewDesignEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsViewDesignEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsViewDesignEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsViewDesignEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsViewDesignEN objInFor in arrObj)
{
//Rows
...
if (clsViewDesignWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsViewDesignEN._CurrTabName);
CacheHelper.Remove(strKey);
clsViewDesignWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsViewDesignEN._CurrTabName;
List<clsViewDesignEN> arrViewDesignObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsViewDesignEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conViewDesign.ViewDesignId, Type.GetType("System.String"));
objDT.Columns.Add(conViewDesign.ViewDesignName, Type.GetType("System.String"));
objDT.Columns.Add(conViewDesign.Memo, Type.GetType("System.String"));
foreach (clsViewDesignEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conViewDesign.ViewDesignId] = objInFor[conViewDesign.ViewDesignId];
objDR[conViewDesign.ViewDesignName] = objInFor[conViewDesign.ViewDesignName];
objDR[conViewDesign.Memo] = objInFor[conViewDesign.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4ViewDesign : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsViewDesignWApi.ReFreshThisCache();
}
}
