...
clsViewReferFilesWApi.ReFreshCache(objViewReferFilesEN.PrjId);
return bolResult;
}
...
from objViewReferFilesEN in arrViewReferFilesObjLstCache
where objViewReferFilesEN.mId == lngmId 
select objViewReferFilesEN;
if (arrViewReferFilesObjLst_Sel.Count() == 0)
{
   clsViewReferFilesEN obj = clsViewReferFilesWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsViewReferFilesEN> GetObjLstByMIdLstCache(List<long> arrMId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsViewReferFilesEN._CurrTabName, strPrjId);
List<clsViewReferFilesEN> arrViewReferFilesObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsViewReferFilesEN> arrViewReferFilesObjLst_Sel =
from objViewReferFilesEN in arrViewReferFilesObjLstCache
where arrMId.Contains(objViewReferFilesEN.mId)
select objViewReferFilesEN;
...
if ((int)jobjReturn0["errorId"] == 0)
{
 clsViewReferFilesEN objViewReferFilesEN = clsViewReferFilesWApi.GetObjBymId(long.Parse(arrmId[0]));
clsViewReferFilesWApi.ReFreshCache(objViewReferFilesEN.PrjId);
var intReturnInt = (int)jobjReturn0["returnInt"];
return intReturnInt;
}
...
if (clsViewReferFilesEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsViewReferFilesEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsViewReferFilesWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsViewReferFilesWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsViewReferFilesWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsViewReferFilesWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsViewReferFilesEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsViewReferFilesEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",conViewReferFiles.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsViewReferFilesEN._CurrTabName, strPrjId);
List<clsViewReferFilesEN> arrViewReferFilesObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrViewReferFilesObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsViewReferFilesEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsViewReferFilesEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsViewReferFilesWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsViewReferFilesWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsViewReferFilesEN._CurrTabName, strPrjId);
List<clsViewReferFilesEN> arrViewReferFilesObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrViewReferFilesObjLstCache = CacheHelper.Get<List<clsViewReferFilesEN>>(strKey);
}
else
{
