...
where objCodeTypeEN.CodeTypeId == strCodeTypeId 
select objCodeTypeEN;
if (arrCodeTypeObjLst_Sel.Count() == 0)
{
   clsCodeTypeEN obj = clsCodeTypeWApi.GetObjByCodeTypeId(strCodeTypeId);
   if (obj != null)
 {
...
if (arrCodeTypeObjLst_Sel.Count > 0)
{
return arrCodeTypeObjLst_Sel[0].CodeTypeName;
}
string strErrMsgForGetObjById = string.Format("在CodeType对象缓存列表中,找不到记录[CodeTypeId = {0}](函数:{1})", strCodeTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsCodeTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在CodeType对象缓存列表中,找不到记录的相关名称[CodeTypeId = {0}](函数:{1})", strCodeTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsCodeTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
public static IEnumerable<clsCodeTypeEN> GetObjLstByCodeTypeIdLstCache(List<string> arrCodeTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsCodeTypeEN._CurrTabName);
List<clsCodeTypeEN> arrCodeTypeObjLstCache = GetObjLstCache();
IEnumerable <clsCodeTypeEN> arrCodeTypeObjLst_Sel =
from objCodeTypeEN in arrCodeTypeObjLstCache
where arrCodeTypeId.Contains(objCodeTypeEN.CodeTypeId)
select objCodeTypeEN;
...
if (clsCodeTypeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsCodeTypeEN._AttributeName)
{
...
var strKey = clsCodeTypeEN._CurrTabName;
List<clsCodeTypeEN> arrCodeTypeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
