...
/// 静态类
/// </summary>
public static class  clsReferFileTypeWApi_Static
{
...
return objReferFileTypeEN;
 ...
 /// </summary>
 /// <param name = "objReferFileTypeEN">需要设置字段值的实体对象</param>
 /// <param name = "strReferFileTypeENName">引用文件类型英文名</param>
 /// <param name = "strComparisonOp">比较运算符,如果有值,可用于组织条件串</param>
 /// <returns>返回对象,可以继续连写</returns>
...
try
{
objReferFileTypeEN.sfUpdFldSetStr = objReferFileTypeEN.getsfUpdFldSetStr();
clsReferFileTypeWApi.CheckPropertyNew(objReferFileTypeEN); 
bool bolResult = clsReferFileTypeWApi.UpdateRecord(objReferFileTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsReferFileTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsReferFileTypeEN objReferFileTypeEN)
{
//检测记录是否存在
...
public static bool AddNewRecord(this clsReferFileTypeEN objReferFileTypeEN)
{
try
{
 if (string.IsNullOrEmpty(objReferFileTypeEN.ReferFileTypeId) == true || clsReferFileTypeWApi.IsExist(objReferFileTypeEN.ReferFileTypeId) == true)
 {
     objReferFileTypeEN.ReferFileTypeId = clsReferFileTypeWApi.GetMaxStrId();
 }
clsReferFileTypeWApi.CheckPropertyNew(objReferFileTypeEN); 
bool bolResult = clsReferFileTypeWApi.AddNewRecord(objReferFileTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsReferFileTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objReferFileTypeEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsReferFileTypeEN objReferFileTypeEN)
{
try
{
clsReferFileTypeWApi.CheckPropertyNew(objReferFileTypeEN); 
string strReferFileTypeId = clsReferFileTypeWApi.AddNewRecordWithMaxId(objReferFileTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsReferFileTypeWApi.ReFreshCache();
...
{
clsReferFileTypeWApi.CheckPropertyNew(objReferFileTypeEN); 
bool bolResult = clsReferFileTypeWApi.UpdateWithCondition(objReferFileTypeEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsReferFileTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class enumReferFileTypeWA
{
 /// <summary>
 /// JsvaScript
 /// </summary>
...
public const string Style_02 = "02";
 ...
 /// </summary>
public class clsReferFileTypeWApi
{
private static readonly string mstrApiControllerName = "ReferFileTypeApi";
...
List<clsReferFileTypeEN> arrObjLst = GetObjLst("1=1");
objDDL.DataValueField="ReferFileTypeId";
objDDL.DataTextField="ReferFileTypeName";
...
List<clsReferFileTypeEN> arrObjLst = clsReferFileTypeWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
if (!Object.Equals(null, objReferFileTypeEN.ReferFileTypeId) && GetStrLen(objReferFileTypeEN.ReferFileTypeId) > 2)
{
 throw new Exception("字段[引用文件类型Id]的长度不能超过2!");
}
if (!Object.Equals(null, objReferFileTypeEN.ReferFileTypeName) && GetStrLen(objReferFileTypeEN.ReferFileTypeName) > 50)
{
 throw new Exception("字段[引用文件类型名]的长度不能超过50!");
}
if (!Object.Equals(null, objReferFileTypeEN.ReferFileTypeENName) && GetStrLen(objReferFileTypeEN.ReferFileTypeENName) > 50)
{
 throw new Exception("字段[引用文件类型英文名]的长度不能超过50!");
}
if (!Object.Equals(null, objReferFileTypeEN.UpdDate) && GetStrLen(objReferFileTypeEN.UpdDate) > 20)
{
 throw new Exception("字段[修改日期]的长度不能超过20!");
}
if (!Object.Equals(null, objReferFileTypeEN.UpdUserId) && GetStrLen(objReferFileTypeEN.UpdUserId) > 20)
{
 throw new Exception("字段[修改用户Id]的长度不能超过20!");
}
if (!Object.Equals(null, objReferFileTypeEN.Memo) && GetStrLen(objReferFileTypeEN.Memo) > 1000)
{
...
{
if (strReferFileTypeId == "") return null;
string strAction = "GetObjByReferFileTypeId";
clsReferFileTypeEN objReferFileTypeEN;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strReferFileTypeId"] = strReferFileTypeId,
};
try
{
...
objReferFileTypeEN = JsonConvert.DeserializeObject<clsReferFileTypeEN>(strJson);
return objReferFileTypeEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsReferFileTypeEN GetFirstObj(string strWhereCond)
{
...
{
if (string.IsNullOrEmpty(strReferFileTypeId) == true) return null;
//初始化列表缓存
...
from objReferFileTypeEN in arrReferFileTypeObjLstCache
where objReferFileTypeEN.ReferFileTypeId == strReferFileTypeId 
select objReferFileTypeEN;
if (arrReferFileTypeObjLst_Sel.Count() == 0)
{
   clsReferFileTypeEN obj = clsReferFileTypeWApi.GetObjByReferFileTypeId(strReferFileTypeId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsReferFileTypeEN> arrReferFileTypeObjLstCache = GetObjLstCache();
IEnumerable <clsReferFileTypeEN> arrReferFileTypeObjLst_Sel1 =
from objReferFileTypeEN in arrReferFileTypeObjLstCache
where objReferFileTypeEN.ReferFileTypeId == strReferFileTypeId 
select objReferFileTypeEN;
List <clsReferFileTypeEN> arrReferFileTypeObjLst_Sel = new List<clsReferFileTypeEN>();
foreach (clsReferFileTypeEN obj in arrReferFileTypeObjLst_Sel1)
{
arrReferFileTypeObjLst_Sel.Add(obj);
}
if (arrReferFileTypeObjLst_Sel.Count > 0)
{
return arrReferFileTypeObjLst_Sel[0].ReferFileTypeName;
}
string strErrMsgForGetObjById = string.Format("在ReferFileType对象缓存列表中,找不到记录[ReferFileTypeId = {0}](函数:{1})", strReferFileTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsReferFileTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在ReferFileType对象缓存列表中,找不到记录的相关名称[ReferFileTypeId = {0}](函数:{1})", strReferFileTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsReferFileTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
{
 List<clsReferFileTypeEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsReferFileTypeEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrReferFileTypeId">关键字列表</param>
 /// <returns>返回对象列表</returns>
public static List<clsReferFileTypeEN> GetObjLstByReferFileTypeIdLst(List<string> arrReferFileTypeId)
{
 List<clsReferFileTypeEN> arrObjLst; 
string strAction = "GetObjLstByReferFileTypeIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrReferFileTypeId);
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrReferFileTypeId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsReferFileTypeEN> GetObjLstByReferFileTypeIdLstCache(List<string> arrReferFileTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsReferFileTypeEN._CurrTabName);
List<clsReferFileTypeEN> arrReferFileTypeObjLstCache = GetObjLstCache();
IEnumerable <clsReferFileTypeEN> arrReferFileTypeObjLst_Sel =
from objReferFileTypeEN in arrReferFileTypeObjLstCache
where arrReferFileTypeId.Contains(objReferFileTypeEN.ReferFileTypeId)
select objReferFileTypeEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsReferFileTypeEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsReferFileTypeEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsReferFileTypeEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsReferFileTypeEN> arrObjLst; 
...
try
{
 clsReferFileTypeEN objReferFileTypeEN = clsReferFileTypeWApi.GetObjByReferFileTypeId(strReferFileTypeId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strReferFileTypeId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelReferFileTypes(List<string> arrReferFileTypeId)
{
string strAction = "DelReferFileTypes";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrReferFileTypeId);
...
{
string strAction = "DelReferFileTypesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsReferFileTypeEN objReferFileTypeEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsReferFileTypeEN>(objReferFileTypeEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsReferFileTypeEN objReferFileTypeEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsReferFileTypeWApi.ReFreshCache();
var strReferFileTypeId = (string)jobjReturn0["returnStr"];
return strReferFileTypeId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsReferFileTypeEN objReferFileTypeEN)
{
if (string.IsNullOrEmpty(objReferFileTypeEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objReferFileTypeEN.ReferFileTypeId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsReferFileTypeEN objReferFileTypeEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objReferFileTypeEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsReferFileTypeEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsReferFileTypeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsReferFileTypeEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsReferFileTypeEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsReferFileTypeEN objInFor in arrObj)
{
//Rows
...
if (clsReferFileTypeWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsReferFileTypeEN._CurrTabName);
CacheHelper.Remove(strKey);
clsReferFileTypeWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsReferFileTypeEN._CurrTabName;
List<clsReferFileTypeEN> arrReferFileTypeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsReferFileTypeEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conReferFileType.ReferFileTypeId, Type.GetType("System.String"));
objDT.Columns.Add(conReferFileType.ReferFileTypeName, Type.GetType("System.String"));
objDT.Columns.Add(conReferFileType.ReferFileTypeENName, Type.GetType("System.String"));
objDT.Columns.Add(conReferFileType.InUse, Type.GetType("System.Boolean"));
objDT.Columns.Add(conReferFileType.OrderNum, Type.GetType("System.Int32"));
objDT.Columns.Add(conReferFileType.UpdDate, Type.GetType("System.String"));
objDT.Columns.Add(conReferFileType.UpdUserId, Type.GetType("System.String"));
objDT.Columns.Add(conReferFileType.Memo, Type.GetType("System.String"));
foreach (clsReferFileTypeEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conReferFileType.ReferFileTypeId] = objInFor[conReferFileType.ReferFileTypeId];
objDR[conReferFileType.ReferFileTypeName] = objInFor[conReferFileType.ReferFileTypeName];
objDR[conReferFileType.ReferFileTypeENName] = objInFor[conReferFileType.ReferFileTypeENName];
objDR[conReferFileType.InUse] = objInFor[conReferFileType.InUse];
objDR[conReferFileType.OrderNum] = objInFor[conReferFileType.OrderNum];
objDR[conReferFileType.UpdDate] = objInFor[conReferFileType.UpdDate];
objDR[conReferFileType.UpdUserId] = objInFor[conReferFileType.UpdUserId];
objDR[conReferFileType.Memo] = objInFor[conReferFileType.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4ReferFileType : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsReferFileTypeWApi.ReFreshThisCache();
}
}
