...
from objvDict4GCEN in arrvDict4GCObjLstCache
where objvDict4GCEN.DictId == strDictId 
select objvDict4GCEN;
if (arrvDict4GCObjLst_Sel.Count() == 0)
{
   clsvDict4GCEN obj = clsvDict4GCWApi.GetObjByDictId(strDictId);
   if (obj != null)
 {
...
public static IEnumerable<clsvDict4GCEN> GetObjLstByDictIdLstCache(List<string> arrDictId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvDict4GCEN._CurrTabName);
List<clsvDict4GCEN> arrvDict4GCObjLstCache = GetObjLstCache();
IEnumerable <clsvDict4GCEN> arrvDict4GCObjLst_Sel =
from objvDict4GCEN in arrvDict4GCObjLstCache
where arrDictId.Contains(objvDict4GCEN.DictId)
select objvDict4GCEN;
...
if (clsvDict4GCEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvDict4GCEN._AttributeName)
{
...
var strKey = clsvDict4GCEN._CurrTabName;
List<clsvDict4GCEN> arrvDict4GCObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
