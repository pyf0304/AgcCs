...
clsLog4GeneTabCodeWApi.ReFreshCache();
return bolResult;
}
...
from objLog4GeneTabCodeEN in arrLog4GeneTabCodeObjLstCache
where objLog4GeneTabCodeEN.mId == lngmId 
select objLog4GeneTabCodeEN;
if (arrLog4GeneTabCodeObjLst_Sel.Count() == 0)
{
   clsLog4GeneTabCodeEN obj = clsLog4GeneTabCodeWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsLog4GeneTabCodeEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsLog4GeneTabCodeEN._CurrTabName);
List<clsLog4GeneTabCodeEN> arrLog4GeneTabCodeObjLstCache = GetObjLstCache();
IEnumerable <clsLog4GeneTabCodeEN> arrLog4GeneTabCodeObjLst_Sel =
from objLog4GeneTabCodeEN in arrLog4GeneTabCodeObjLstCache
where arrMId.Contains(objLog4GeneTabCodeEN.mId)
select objLog4GeneTabCodeEN;
...
if (clsLog4GeneTabCodeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsLog4GeneTabCodeEN._AttributeName)
{
...
var strKey = clsLog4GeneTabCodeEN._CurrTabName;
List<clsLog4GeneTabCodeEN> arrLog4GeneTabCodeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
