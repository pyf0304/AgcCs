...
from objSysLogEN in arrSysLogObjLstCache
where objSysLogEN.mId == lngmId 
select objSysLogEN;
if (arrSysLogObjLst_Sel.Count() == 0)
{
   clsSysLogEN obj = clsSysLogWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsSysLogEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsSysLogEN._CurrTabName);
List<clsSysLogEN> arrSysLogObjLstCache = GetObjLstCache();
IEnumerable <clsSysLogEN> arrSysLogObjLst_Sel =
from objSysLogEN in arrSysLogObjLstCache
where arrMId.Contains(objSysLogEN.mId)
select objSysLogEN;
...
if (clsSysLogEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsSysLogEN._AttributeName)
{
...
var strKey = clsSysLogEN._CurrTabName;
List<clsSysLogEN> arrSysLogObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
