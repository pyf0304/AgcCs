...
from objvPrjTab4GeneCodeEN in arrvPrjTab4GeneCodeObjLstCache
where objvPrjTab4GeneCodeEN.TabId == strTabId 
select objvPrjTab4GeneCodeEN;
if (arrvPrjTab4GeneCodeObjLst_Sel.Count() == 0)
{
   clsvPrjTab4GeneCodeEN obj = clsvPrjTab4GeneCodeWApi.GetObjByTabId(strTabId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLst_Sel1 =
from objvPrjTab4GeneCodeEN in arrvPrjTab4GeneCodeObjLstCache
where objvPrjTab4GeneCodeEN.TabId == strTabId 
select objvPrjTab4GeneCodeEN;
List <clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLst_Sel = new List<clsvPrjTab4GeneCodeEN>();
foreach (clsvPrjTab4GeneCodeEN obj in arrvPrjTab4GeneCodeObjLst_Sel1)
{
arrvPrjTab4GeneCodeObjLst_Sel.Add(obj);
}
if (arrvPrjTab4GeneCodeObjLst_Sel.Count > 0)
{
return arrvPrjTab4GeneCodeObjLst_Sel[0].TabName;
}
string strErrMsgForGetObjById = string.Format("在vPrjTab4GeneCode对象缓存列表中,找不到记录[TabId = {0}](函数:{1})", strTabId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsvPrjTab4GeneCodeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在vPrjTab4GeneCode对象缓存列表中,找不到记录的相关名称[TabId = {0}](函数:{1})", strTabId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsvPrjTab4GeneCodeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
public static IEnumerable<clsvPrjTab4GeneCodeEN> GetObjLstByTabIdLstCache(List<string> arrTabId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsvPrjTab4GeneCodeEN._CurrTabName, strPrjId);
List<clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLst_Sel =
from objvPrjTab4GeneCodeEN in arrvPrjTab4GeneCodeObjLstCache
where arrTabId.Contains(objvPrjTab4GeneCodeEN.TabId)
select objvPrjTab4GeneCodeEN;
...
if (clsvPrjTab4GeneCodeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvPrjTab4GeneCodeEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvPrjTab4GeneCodeWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvPrjTab4GeneCodeWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvPrjTab4GeneCodeWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvPrjTab4GeneCodeWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsvPrjTab4GeneCodeEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsvPrjTab4GeneCodeEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",convPrjTab4GeneCode.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsvPrjTab4GeneCodeEN._CurrTabName, strPrjId);
List<clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrvPrjTab4GeneCodeObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsvPrjTab4GeneCodeEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsvPrjTab4GeneCodeEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvPrjTab4GeneCodeWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvPrjTab4GeneCodeWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsvPrjTab4GeneCodeEN._CurrTabName, strPrjId);
List<clsvPrjTab4GeneCodeEN> arrvPrjTab4GeneCodeObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrvPrjTab4GeneCodeObjLstCache = CacheHelper.Get<List<clsvPrjTab4GeneCodeEN>>(strKey);
}
else
{
