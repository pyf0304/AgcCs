...
from objvViewInfo4GeneCodeEN in arrvViewInfo4GeneCodeObjLstCache
where objvViewInfo4GeneCodeEN.ViewId == strViewId 
select objvViewInfo4GeneCodeEN;
if (arrvViewInfo4GeneCodeObjLst_Sel.Count() == 0)
{
   clsvViewInfo4GeneCodeEN obj = clsvViewInfo4GeneCodeWApi.GetObjByViewId(strViewId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLst_Sel1 =
from objvViewInfo4GeneCodeEN in arrvViewInfo4GeneCodeObjLstCache
where objvViewInfo4GeneCodeEN.ViewId == strViewId 
select objvViewInfo4GeneCodeEN;
List <clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLst_Sel = new List<clsvViewInfo4GeneCodeEN>();
foreach (clsvViewInfo4GeneCodeEN obj in arrvViewInfo4GeneCodeObjLst_Sel1)
{
arrvViewInfo4GeneCodeObjLst_Sel.Add(obj);
}
if (arrvViewInfo4GeneCodeObjLst_Sel.Count > 0)
{
return arrvViewInfo4GeneCodeObjLst_Sel[0].ViewName;
}
string strErrMsgForGetObjById = string.Format("在vViewInfo4GeneCode对象缓存列表中,找不到记录[ViewId = {0}](函数:{1})", strViewId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsvViewInfo4GeneCodeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在vViewInfo4GeneCode对象缓存列表中,找不到记录的相关名称[ViewId = {0}](函数:{1})", strViewId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsvViewInfo4GeneCodeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
public static IEnumerable<clsvViewInfo4GeneCodeEN> GetObjLstByViewIdLstCache(List<string> arrViewId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsvViewInfo4GeneCodeEN._CurrTabName, strPrjId);
List<clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLst_Sel =
from objvViewInfo4GeneCodeEN in arrvViewInfo4GeneCodeObjLstCache
where arrViewId.Contains(objvViewInfo4GeneCodeEN.ViewId)
select objvViewInfo4GeneCodeEN;
...
if (clsvViewInfo4GeneCodeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvViewInfo4GeneCodeEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvViewInfo4GeneCodeWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvViewInfo4GeneCodeWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvViewInfo4GeneCodeWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvViewInfo4GeneCodeWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsvViewInfo4GeneCodeEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsvViewInfo4GeneCodeEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",convViewInfo4GeneCode.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsvViewInfo4GeneCodeEN._CurrTabName, strPrjId);
List<clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrvViewInfo4GeneCodeObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsvViewInfo4GeneCodeEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsvViewInfo4GeneCodeEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvViewInfo4GeneCodeWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvViewInfo4GeneCodeWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsvViewInfo4GeneCodeEN._CurrTabName, strPrjId);
List<clsvViewInfo4GeneCodeEN> arrvViewInfo4GeneCodeObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrvViewInfo4GeneCodeObjLstCache = CacheHelper.Get<List<clsvViewInfo4GeneCodeEN>>(strKey);
}
else
{
