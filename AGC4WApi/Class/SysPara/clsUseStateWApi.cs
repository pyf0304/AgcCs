...
/// 静态类
/// </summary>
public static class  clsUseStateWApi_Static
{
...
return objUseStateEN;
 ...
 /// </summary>
 /// <param name = "objUseStateEN">需要设置字段值的实体对象</param>
 /// <param name = "strUseStateENName">使用状态英文名</param>
 /// <param name = "strComparisonOp">比较运算符,如果有值,可用于组织条件串</param>
 /// <returns>返回对象,可以继续连写</returns>
...
try
{
objUseStateEN.sfUpdFldSetStr = objUseStateEN.getsfUpdFldSetStr();
clsUseStateWApi.CheckPropertyNew(objUseStateEN); 
bool bolResult = clsUseStateWApi.UpdateRecord(objUseStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUseStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsUseStateEN objUseStateEN)
{
//检测记录是否存在
...
public static bool AddNewRecord(this clsUseStateEN objUseStateEN)
{
try
{
 if (string.IsNullOrEmpty(objUseStateEN.UseStateId) == true || clsUseStateWApi.IsExist(objUseStateEN.UseStateId) == true)
 {
     objUseStateEN.UseStateId = clsUseStateWApi.GetMaxStrId();
 }
clsUseStateWApi.CheckPropertyNew(objUseStateEN); 
bool bolResult = clsUseStateWApi.AddNewRecord(objUseStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUseStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objUseStateEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsUseStateEN objUseStateEN)
{
try
{
clsUseStateWApi.CheckPropertyNew(objUseStateEN); 
string strUseStateId = clsUseStateWApi.AddNewRecordWithMaxId(objUseStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUseStateWApi.ReFreshCache();
...
{
clsUseStateWApi.CheckPropertyNew(objUseStateEN); 
bool bolResult = clsUseStateWApi.UpdateWithCondition(objUseStateEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUseStateWApi.ReFreshCache();
return bolResult;
}
...
public const string NeverUse_0003 = "0003";
 ...
 /// </summary>
public class clsUseStateWApi
{
private static readonly string mstrApiControllerName = "UseStateApi";
...
List<clsUseStateEN> arrObjLst = GetObjLst("1=1");
objDDL.DataValueField="UseStateId";
objDDL.DataTextField="UseStateName";
...
List<clsUseStateEN> arrObjLst = clsUseStateWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
if (!Object.Equals(null, objUseStateEN.UseStateId) && GetStrLen(objUseStateEN.UseStateId) > 4)
{
 throw new Exception("字段[使用状态Id]的长度不能超过4!");
}
if (!Object.Equals(null, objUseStateEN.UseStateName) && GetStrLen(objUseStateEN.UseStateName) > 30)
{
 throw new Exception("字段[使用状态名称]的长度不能超过30!");
}
if (!Object.Equals(null, objUseStateEN.UseStateENName) && GetStrLen(objUseStateEN.UseStateENName) > 30)
{
 throw new Exception("字段[使用状态英文名]的长度不能超过30!");
}
if (!Object.Equals(null, objUseStateEN.Memo) && GetStrLen(objUseStateEN.Memo) > 1000)
{
...
{
if (strUseStateId == "") return null;
string strAction = "GetObjByUseStateId";
clsUseStateEN objUseStateEN;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strUseStateId"] = strUseStateId,
};
try
{
...
objUseStateEN = JsonConvert.DeserializeObject<clsUseStateEN>(strJson);
return objUseStateEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsUseStateEN GetFirstObj(string strWhereCond)
{
...
{
if (string.IsNullOrEmpty(strUseStateId) == true) return null;
//初始化列表缓存
...
from objUseStateEN in arrUseStateObjLstCache
where objUseStateEN.UseStateId == strUseStateId 
select objUseStateEN;
if (arrUseStateObjLst_Sel.Count() == 0)
{
   clsUseStateEN obj = clsUseStateWApi.GetObjByUseStateId(strUseStateId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsUseStateEN> arrUseStateObjLstCache = GetObjLstCache();
IEnumerable <clsUseStateEN> arrUseStateObjLst_Sel1 =
from objUseStateEN in arrUseStateObjLstCache
where objUseStateEN.UseStateId == strUseStateId 
select objUseStateEN;
List <clsUseStateEN> arrUseStateObjLst_Sel = new List<clsUseStateEN>();
foreach (clsUseStateEN obj in arrUseStateObjLst_Sel1)
{
arrUseStateObjLst_Sel.Add(obj);
}
if (arrUseStateObjLst_Sel.Count > 0)
{
return arrUseStateObjLst_Sel[0].UseStateName;
}
string strErrMsgForGetObjById = string.Format("在UseState对象缓存列表中,找不到记录[UseStateId = {0}](函数:{1})", strUseStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsUseStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在UseState对象缓存列表中,找不到记录的相关名称[UseStateId = {0}](函数:{1})", strUseStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsUseStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
arrObjLst = JsonConvert.DeserializeObject<List<clsUseStateEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrUseStateId">关键字列表</param>
 /// <returns>返回对象列表</returns>
public static List<clsUseStateEN> GetObjLstByUseStateIdLst(List<string> arrUseStateId)
{
 List<clsUseStateEN> arrObjLst; 
string strAction = "GetObjLstByUseStateIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrUseStateId);
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrUseStateId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsUseStateEN> GetObjLstByUseStateIdLstCache(List<string> arrUseStateId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsUseStateEN._CurrTabName);
List<clsUseStateEN> arrUseStateObjLstCache = GetObjLstCache();
IEnumerable <clsUseStateEN> arrUseStateObjLst_Sel =
from objUseStateEN in arrUseStateObjLstCache
where arrUseStateId.Contains(objUseStateEN.UseStateId)
select objUseStateEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsUseStateEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsUseStateEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsUseStateEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsUseStateEN> arrObjLst; 
...
try
{
 clsUseStateEN objUseStateEN = clsUseStateWApi.GetObjByUseStateId(strUseStateId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strUseStateId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelUseStates(List<string> arrUseStateId)
{
string strAction = "DelUseStates";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrUseStateId);
...
{
string strAction = "DelUseStatesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsUseStateEN objUseStateEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsUseStateEN>(objUseStateEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsUseStateEN objUseStateEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUseStateWApi.ReFreshCache();
var strUseStateId = (string)jobjReturn0["returnStr"];
return strUseStateId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsUseStateEN objUseStateEN)
{
if (string.IsNullOrEmpty(objUseStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objUseStateEN.UseStateId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsUseStateEN objUseStateEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objUseStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsUseStateEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsUseStateEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsUseStateEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsUseStateEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsUseStateEN objInFor in arrObj)
{
//Rows
...
if (clsUseStateWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsUseStateEN._CurrTabName);
CacheHelper.Remove(strKey);
clsUseStateWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsUseStateEN._CurrTabName;
List<clsUseStateEN> arrUseStateObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsUseStateEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conUseState.UseStateId, Type.GetType("System.String"));
objDT.Columns.Add(conUseState.UseStateName, Type.GetType("System.String"));
objDT.Columns.Add(conUseState.UseStateENName, Type.GetType("System.String"));
objDT.Columns.Add(conUseState.Memo, Type.GetType("System.String"));
foreach (clsUseStateEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conUseState.UseStateId] = objInFor[conUseState.UseStateId];
objDR[conUseState.UseStateName] = objInFor[conUseState.UseStateName];
objDR[conUseState.UseStateENName] = objInFor[conUseState.UseStateENName];
objDR[conUseState.Memo] = objInFor[conUseState.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4UseState : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUseStateWApi.ReFreshThisCache();
}
}
