...
{
if (strTitleStyleId == "") return null;
...
from objvTitleStyleEN in arrvTitleStyleObjLstCache
where objvTitleStyleEN.TitleStyleId == strTitleStyleId 
select objvTitleStyleEN;
if (arrvTitleStyleObjLst_Sel.Count() == 0)
{
   clsvTitleStyleEN obj = clsvTitleStyleWApi.GetObjByTitleStyleId(strTitleStyleId);
   if (obj != null)
 {
...
public static IEnumerable<clsvTitleStyleEN> GetObjLstByTitleStyleIdLstCache(List<string> arrTitleStyleId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvTitleStyleEN._CurrTabName);
List<clsvTitleStyleEN> arrvTitleStyleObjLstCache = GetObjLstCache();
IEnumerable <clsvTitleStyleEN> arrvTitleStyleObjLst_Sel =
from objvTitleStyleEN in arrvTitleStyleObjLstCache
where arrTitleStyleId.Contains(objvTitleStyleEN.TitleStyleId)
select objvTitleStyleEN;
...
if (clsvTitleStyleEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvTitleStyleEN._AttributeName)
{
...
var strKey = clsvTitleStyleEN._CurrTabName;
List<clsvTitleStyleEN> arrvTitleStyleObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
