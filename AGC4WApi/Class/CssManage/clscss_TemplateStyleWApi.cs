...
clscss_TemplateStyleWApi.ReFreshCache();
return bolResult;
}
...
from objcss_TemplateStyleEN in arrcss_TemplateStyleObjLstCache
where objcss_TemplateStyleEN.mId == lngmId 
select objcss_TemplateStyleEN;
if (arrcss_TemplateStyleObjLst_Sel.Count() == 0)
{
   clscss_TemplateStyleEN obj = clscss_TemplateStyleWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clscss_TemplateStyleEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clscss_TemplateStyleEN._CurrTabName);
List<clscss_TemplateStyleEN> arrcss_TemplateStyleObjLstCache = GetObjLstCache();
IEnumerable <clscss_TemplateStyleEN> arrcss_TemplateStyleObjLst_Sel =
from objcss_TemplateStyleEN in arrcss_TemplateStyleObjLstCache
where arrMId.Contains(objcss_TemplateStyleEN.mId)
select objcss_TemplateStyleEN;
...
if (clscss_TemplateStyleEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clscss_TemplateStyleEN._AttributeName)
{
...
var strKey = clscss_TemplateStyleEN._CurrTabName;
List<clscss_TemplateStyleEN> arrcss_TemplateStyleObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
