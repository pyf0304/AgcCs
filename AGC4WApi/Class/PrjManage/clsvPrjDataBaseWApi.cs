...
{
if (strPrjDataBaseId == "") return null;
...
from objvPrjDataBaseEN in arrvPrjDataBaseObjLstCache
where objvPrjDataBaseEN.PrjDataBaseId == strPrjDataBaseId 
select objvPrjDataBaseEN;
if (arrvPrjDataBaseObjLst_Sel.Count() == 0)
{
   clsvPrjDataBaseEN obj = clsvPrjDataBaseWApi.GetObjByPrjDataBaseId(strPrjDataBaseId);
   if (obj != null)
 {
...
public static IEnumerable<clsvPrjDataBaseEN> GetObjLstByPrjDataBaseIdLstCache(List<string> arrPrjDataBaseId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvPrjDataBaseEN._CurrTabName);
List<clsvPrjDataBaseEN> arrvPrjDataBaseObjLstCache = GetObjLstCache();
IEnumerable <clsvPrjDataBaseEN> arrvPrjDataBaseObjLst_Sel =
from objvPrjDataBaseEN in arrvPrjDataBaseObjLstCache
where arrPrjDataBaseId.Contains(objvPrjDataBaseEN.PrjDataBaseId)
select objvPrjDataBaseEN;
...
if (clsvPrjDataBaseEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvPrjDataBaseEN._AttributeName)
{
...
var strKey = clsvPrjDataBaseEN._CurrTabName;
List<clsvPrjDataBaseEN> arrvPrjDataBaseObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
