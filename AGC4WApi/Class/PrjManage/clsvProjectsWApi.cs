...
from objvProjectsEN in arrvProjectsObjLstCache
where objvProjectsEN.PrjId == strPrjId 
select objvProjectsEN;
if (arrvProjectsObjLst_Sel.Count() == 0)
{
   clsvProjectsEN obj = clsvProjectsWApi.GetObjByPrjId(strPrjId);
   if (obj != null)
 {
...
public static IEnumerable<clsvProjectsEN> GetObjLstByPrjIdLstCache(List<string> arrPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvProjectsEN._CurrTabName);
List<clsvProjectsEN> arrvProjectsObjLstCache = GetObjLstCache();
IEnumerable <clsvProjectsEN> arrvProjectsObjLst_Sel =
from objvProjectsEN in arrvProjectsObjLstCache
where arrPrjId.Contains(objvProjectsEN.PrjId)
select objvProjectsEN;
...
if (clsvProjectsEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvProjectsEN._AttributeName)
{
...
var strKey = clsvProjectsEN._CurrTabName;
List<clsvProjectsEN> arrvProjectsObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
