...
from objvPrjCasesEN in arrvPrjCasesObjLstCache
where objvPrjCasesEN.CaseId == strCaseId 
select objvPrjCasesEN;
if (arrvPrjCasesObjLst_Sel.Count() == 0)
{
   clsvPrjCasesEN obj = clsvPrjCasesWApi.GetObjByCaseId(strCaseId);
   if (obj != null)
 {
...
public static IEnumerable<clsvPrjCasesEN> GetObjLstByCaseIdLstCache(List<string> arrCaseId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvPrjCasesEN._CurrTabName);
List<clsvPrjCasesEN> arrvPrjCasesObjLstCache = GetObjLstCache();
IEnumerable <clsvPrjCasesEN> arrvPrjCasesObjLst_Sel =
from objvPrjCasesEN in arrvPrjCasesObjLstCache
where arrCaseId.Contains(objvPrjCasesEN.CaseId)
select objvPrjCasesEN;
...
if (clsvPrjCasesEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvPrjCasesEN._AttributeName)
{
...
var strKey = clsvPrjCasesEN._CurrTabName;
List<clsvPrjCasesEN> arrvPrjCasesObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
