...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsCaseTypeEN objCaseTypeEN)
{
//检测记录是否存在
...
{
if (strCaseTypeId == "") return null;
string strAction = "GetObjByCaseTypeId";
clsCaseTypeEN objCaseTypeEN;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strCaseTypeId"] = strCaseTypeId,
};
try
{
...
objCaseTypeEN = JsonConvert.DeserializeObject<clsCaseTypeEN>(strJson);
return objCaseTypeEN;
}
else
{
...
from objCaseTypeEN in arrCaseTypeObjLstCache
...
arrObjLst = JsonConvert.DeserializeObject<List<clsCaseTypeEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrCaseTypeId">关键字列表</param>
 /// <returns>返回对象列表</returns>
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrCaseTypeId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsCaseTypeEN> GetObjLstByCaseTypeIdLstCache(List<string> arrCaseTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsCaseTypeEN._CurrTabName);
List<clsCaseTypeEN> arrCaseTypeObjLstCache = GetObjLstCache();
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsCaseTypeEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsCaseTypeEN> arrObjLst; 
...
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strCaseTypeId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCaseTypeWApi.ReFreshCache();
var strCaseTypeId = (string)jobjReturn0["returnStr"];
return strCaseTypeId;
}
else
{
...
if (clsCaseTypeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsCaseTypeEN._AttributeName)
{
...
var strKey = clsCaseTypeEN._CurrTabName;
List<clsCaseTypeEN> arrCaseTypeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
