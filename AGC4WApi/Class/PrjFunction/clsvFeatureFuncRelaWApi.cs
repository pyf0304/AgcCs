...
from objvFeatureFuncRelaEN in arrvFeatureFuncRelaObjLstCache
where objvFeatureFuncRelaEN.mId == lngmId 
select objvFeatureFuncRelaEN;
if (arrvFeatureFuncRelaObjLst_Sel.Count() == 0)
{
   clsvFeatureFuncRelaEN obj = clsvFeatureFuncRelaWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvFeatureFuncRelaEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvFeatureFuncRelaEN._CurrTabName);
List<clsvFeatureFuncRelaEN> arrvFeatureFuncRelaObjLstCache = GetObjLstCache();
IEnumerable <clsvFeatureFuncRelaEN> arrvFeatureFuncRelaObjLst_Sel =
from objvFeatureFuncRelaEN in arrvFeatureFuncRelaObjLstCache
where arrMId.Contains(objvFeatureFuncRelaEN.mId)
select objvFeatureFuncRelaEN;
...
if (clsvFeatureFuncRelaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvFeatureFuncRelaEN._AttributeName)
{
...
var strKey = clsvFeatureFuncRelaEN._CurrTabName;
List<clsvFeatureFuncRelaEN> arrvFeatureFuncRelaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
