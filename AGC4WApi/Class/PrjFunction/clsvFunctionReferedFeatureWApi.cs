...
from objvFunctionReferedFeatureEN in arrvFunctionReferedFeatureObjLstCache
where objvFunctionReferedFeatureEN.mId == lngmId 
select objvFunctionReferedFeatureEN;
if (arrvFunctionReferedFeatureObjLst_Sel.Count() == 0)
{
   clsvFunctionReferedFeatureEN obj = clsvFunctionReferedFeatureWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvFunctionReferedFeatureEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvFunctionReferedFeatureEN._CurrTabName);
List<clsvFunctionReferedFeatureEN> arrvFunctionReferedFeatureObjLstCache = GetObjLstCache();
IEnumerable <clsvFunctionReferedFeatureEN> arrvFunctionReferedFeatureObjLst_Sel =
from objvFunctionReferedFeatureEN in arrvFunctionReferedFeatureObjLstCache
where arrMId.Contains(objvFunctionReferedFeatureEN.mId)
select objvFunctionReferedFeatureEN;
...
if (clsvFunctionReferedFeatureEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvFunctionReferedFeatureEN._AttributeName)
{
...
var strKey = clsvFunctionReferedFeatureEN._CurrTabName;
List<clsvFunctionReferedFeatureEN> arrvFunctionReferedFeatureObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
