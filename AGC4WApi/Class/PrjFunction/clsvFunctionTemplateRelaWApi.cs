...
from objvFunctionTemplateRelaEN in arrvFunctionTemplateRelaObjLstCache
...
{
   clsvFunctionTemplateRelaEN obj = clsvFunctionTemplateRelaWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvFunctionTemplateRelaEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvFunctionTemplateRelaEN._CurrTabName);
List<clsvFunctionTemplateRelaEN> arrvFunctionTemplateRelaObjLstCache = GetObjLstCache();
IEnumerable <clsvFunctionTemplateRelaEN> arrvFunctionTemplateRelaObjLst_Sel =
from objvFunctionTemplateRelaEN in arrvFunctionTemplateRelaObjLstCache
where arrMId.Contains(objvFunctionTemplateRelaEN.mId)
select objvFunctionTemplateRelaEN;
...
if (clsvFunctionTemplateRelaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvFunctionTemplateRelaEN._AttributeName)
{
...
var strKey = clsvFunctionTemplateRelaEN._CurrTabName;
List<clsvFunctionTemplateRelaEN> arrvFunctionTemplateRelaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
