...
{
  var strMsg = string.Format("参数:[strFunctionTemplateId]不能为空！(In clsFunctionTemplateRelaWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strFunctionTemplateId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strFunctionTemplateId]的长度:[{0}]不正确！(clsFunctionTemplateRelaWApi.ReFreshThisCache)", strFunctionTemplateId.Length);
throw new Exception (strMsg);
}
string strMsg0;
if (clsSysParaEN.spSetRefreshCacheOn == true)
{
string strKey = string.Format("{0}_{1}", clsFunctionTemplateRelaEN._CurrTabName, strFunctionTemplateId);
...
clsSysParaEN.objLog.WriteDebugLog(strMsg0);
 ...
 /// </summary>
public static void ReFreshCache(string strFunctionTemplateId)
{
...
string strKey = string.Format("{0}_{1}", clsFunctionTemplateRelaEN._CurrTabName, strFunctionTemplateId);
CacheHelper.Remove(strKey);
clsFunctionTemplateRelaWApi.objCommFun4WApi.ReFreshCache(strFunctionTemplateId.ToString());
}
}
