...
from objvFuncParaRelaEN in arrvFuncParaRelaObjLstCache
where objvFuncParaRelaEN.mId == lngmId 
select objvFuncParaRelaEN;
if (arrvFuncParaRelaObjLst_Sel.Count() == 0)
{
   clsvFuncParaRelaEN obj = clsvFuncParaRelaWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvFuncParaRelaEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvFuncParaRelaEN._CurrTabName);
List<clsvFuncParaRelaEN> arrvFuncParaRelaObjLstCache = GetObjLstCache();
IEnumerable <clsvFuncParaRelaEN> arrvFuncParaRelaObjLst_Sel =
from objvFuncParaRelaEN in arrvFuncParaRelaObjLstCache
where arrMId.Contains(objvFuncParaRelaEN.mId)
select objvFuncParaRelaEN;
...
if (clsvFuncParaRelaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvFuncParaRelaEN._AttributeName)
{
...
var strKey = clsvFuncParaRelaEN._CurrTabName;
List<clsvFuncParaRelaEN> arrvFuncParaRelaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
