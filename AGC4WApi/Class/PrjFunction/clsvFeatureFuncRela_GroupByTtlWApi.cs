...
from objvFeatureFuncRela_GroupByTtlEN in arrvFeatureFuncRela_GroupByTtlObjLstCache
where objvFeatureFuncRela_GroupByTtlEN.FeatureId == strFeatureId 
select objvFeatureFuncRela_GroupByTtlEN;
if (arrvFeatureFuncRela_GroupByTtlObjLst_Sel.Count() == 0)
{
   clsvFeatureFuncRela_GroupByTtlEN obj = clsvFeatureFuncRela_GroupByTtlWApi.GetObjByFeatureId(strFeatureId);
   if (obj != null)
 {
...
public static IEnumerable<clsvFeatureFuncRela_GroupByTtlEN> GetObjLstByFeatureIdLstCache(List<string> arrFeatureId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvFeatureFuncRela_GroupByTtlEN._CurrTabName);
List<clsvFeatureFuncRela_GroupByTtlEN> arrvFeatureFuncRela_GroupByTtlObjLstCache = GetObjLstCache();
IEnumerable <clsvFeatureFuncRela_GroupByTtlEN> arrvFeatureFuncRela_GroupByTtlObjLst_Sel =
from objvFeatureFuncRela_GroupByTtlEN in arrvFeatureFuncRela_GroupByTtlObjLstCache
where arrFeatureId.Contains(objvFeatureFuncRela_GroupByTtlEN.FeatureId)
select objvFeatureFuncRela_GroupByTtlEN;
...
if (clsvFeatureFuncRela_GroupByTtlEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvFeatureFuncRela_GroupByTtlEN._AttributeName)
{
...
var strKey = clsvFeatureFuncRela_GroupByTtlEN._CurrTabName;
List<clsvFeatureFuncRela_GroupByTtlEN> arrvFeatureFuncRela_GroupByTtlObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
