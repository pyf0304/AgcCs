...
clsFunction4GeneCodeWApi.ReFreshCache();
return bolResult;
}
...
{
   clsFunction4GeneCodeEN obj = clsFunction4GeneCodeWApi.GetObjByFuncId4GC(strFuncId4GC);
   if (obj != null)
 {
...
if (arrFunction4GeneCodeObjLst_Sel.Count > 0)
{
return arrFunction4GeneCodeObjLst_Sel[0].FuncName;
}
string strErrMsgForGetObjById = string.Format("在Function4GeneCode对象缓存列表中,找不到记录[FuncId4GC = {0}](函数:{1})", strFuncId4GC, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsFunction4GeneCodeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在Function4GeneCode对象缓存列表中,找不到记录的相关名称[FuncId4GC = {0}](函数:{1})", strFuncId4GC, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsFunction4GeneCodeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
public static IEnumerable<clsFunction4GeneCodeEN> GetObjLstByFuncId4GCLstCache(List<string> arrFuncId4GC)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsFunction4GeneCodeEN._CurrTabName);
List<clsFunction4GeneCodeEN> arrFunction4GeneCodeObjLstCache = GetObjLstCache();
IEnumerable <clsFunction4GeneCodeEN> arrFunction4GeneCodeObjLst_Sel =
from objFunction4GeneCodeEN in arrFunction4GeneCodeObjLstCache
where arrFuncId4GC.Contains(objFunction4GeneCodeEN.FuncId4GC)
select objFunction4GeneCodeEN;
...
if (clsFunction4GeneCodeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsFunction4GeneCodeEN._AttributeName)
{
...
var strKey = clsFunction4GeneCodeEN._CurrTabName;
List<clsFunction4GeneCodeEN> arrFunction4GeneCodeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
