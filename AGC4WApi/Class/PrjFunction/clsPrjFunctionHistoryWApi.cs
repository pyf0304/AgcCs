...
clsPrjFunctionHistoryWApi.ReFreshCache();
return bolResult;
}
...
from objPrjFunctionHistoryEN in arrPrjFunctionHistoryObjLstCache
where objPrjFunctionHistoryEN.mId == lngmId 
select objPrjFunctionHistoryEN;
if (arrPrjFunctionHistoryObjLst_Sel.Count() == 0)
{
   clsPrjFunctionHistoryEN obj = clsPrjFunctionHistoryWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsPrjFunctionHistoryEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsPrjFunctionHistoryEN._CurrTabName);
List<clsPrjFunctionHistoryEN> arrPrjFunctionHistoryObjLstCache = GetObjLstCache();
IEnumerable <clsPrjFunctionHistoryEN> arrPrjFunctionHistoryObjLst_Sel =
from objPrjFunctionHistoryEN in arrPrjFunctionHistoryObjLstCache
where arrMId.Contains(objPrjFunctionHistoryEN.mId)
select objPrjFunctionHistoryEN;
...
if (clsPrjFunctionHistoryEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsPrjFunctionHistoryEN._AttributeName)
{
...
var strKey = clsPrjFunctionHistoryEN._CurrTabName;
List<clsPrjFunctionHistoryEN> arrPrjFunctionHistoryObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
