...
public static clsvExcelExportRegionFldCountEN GetObjByRegionIdCache(string strRegionId)
{
...
List<clsvExcelExportRegionFldCountEN> arrvExcelExportRegionFldCountObjLstCache = GetObjLstCache();
IEnumerable <clsvExcelExportRegionFldCountEN> arrvExcelExportRegionFldCountObjLst_Sel =
from objvExcelExportRegionFldCountEN in arrvExcelExportRegionFldCountObjLstCache
where objvExcelExportRegionFldCountEN.RegionId == strRegionId 
select objvExcelExportRegionFldCountEN;
if (arrvExcelExportRegionFldCountObjLst_Sel.Count() == 0)
{
   clsvExcelExportRegionFldCountEN obj = clsvExcelExportRegionFldCountWApi.GetObjByRegionId(strRegionId);
   if (obj != null)
 {
...
public static IEnumerable<clsvExcelExportRegionFldCountEN> GetObjLstByRegionIdLstCache(List<string> arrRegionId, )
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsvExcelExportRegionFldCountEN._CurrTabName, strPrjId);
List<clsvExcelExportRegionFldCountEN> arrvExcelExportRegionFldCountObjLstCache = GetObjLstCache();
IEnumerable <clsvExcelExportRegionFldCountEN> arrvExcelExportRegionFldCountObjLst_Sel =
from objvExcelExportRegionFldCountEN in arrvExcelExportRegionFldCountObjLstCache
where arrRegionId.Contains(objvExcelExportRegionFldCountEN.RegionId)
select objvExcelExportRegionFldCountEN;
...
if (clsvExcelExportRegionFldCountEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvExcelExportRegionFldCountEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvExcelExportRegionFldCountWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvExcelExportRegionFldCountWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
public static List<clsvExcelExportRegionFldCountEN> GetObjLstCache()
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvExcelExportRegionFldCountWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvExcelExportRegionFldCountWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsvExcelExportRegionFldCountEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsvExcelExportRegionFldCountEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",convExcelExportRegionFldCount.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsvExcelExportRegionFldCountEN._CurrTabName, strPrjId);
List<clsvExcelExportRegionFldCountEN> arrvExcelExportRegionFldCountObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrvExcelExportRegionFldCountObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsvExcelExportRegionFldCountEN> GetObjLstCacheFromObjLst(List<clsvExcelExportRegionFldCountEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvExcelExportRegionFldCountWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvExcelExportRegionFldCountWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsvExcelExportRegionFldCountEN._CurrTabName, strPrjId);
List<clsvExcelExportRegionFldCountEN> arrvExcelExportRegionFldCountObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrvExcelExportRegionFldCountObjLstCache = CacheHelper.Get<List<clsvExcelExportRegionFldCountEN>>(strKey);
}
else
{
