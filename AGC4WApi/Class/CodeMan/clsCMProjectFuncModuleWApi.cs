...
clsCMProjectFuncModuleWApi.ReFreshCache(objCMProjectFuncModuleEN.PrjId);
return bolResult;
}
...
from objCMProjectFuncModuleEN in arrCMProjectFuncModuleObjLstCache
where objCMProjectFuncModuleEN.mId == lngmId 
select objCMProjectFuncModuleEN;
if (arrCMProjectFuncModuleObjLst_Sel.Count() == 0)
{
   clsCMProjectFuncModuleEN obj = clsCMProjectFuncModuleWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsCMProjectFuncModuleEN> GetObjLstByMIdLstCache(List<long> arrMId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsCMProjectFuncModuleEN._CurrTabName, strPrjId);
List<clsCMProjectFuncModuleEN> arrCMProjectFuncModuleObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsCMProjectFuncModuleEN> arrCMProjectFuncModuleObjLst_Sel =
from objCMProjectFuncModuleEN in arrCMProjectFuncModuleObjLstCache
where arrMId.Contains(objCMProjectFuncModuleEN.mId)
select objCMProjectFuncModuleEN;
...
if ((int)jobjReturn0["errorId"] == 0)
{
 clsCMProjectFuncModuleEN objCMProjectFuncModuleEN = clsCMProjectFuncModuleWApi.GetObjBymId(long.Parse(arrmId[0]));
clsCMProjectFuncModuleWApi.ReFreshCache(objCMProjectFuncModuleEN.PrjId);
var intReturnInt = (int)jobjReturn0["returnInt"];
return intReturnInt;
}
...
if (clsCMProjectFuncModuleEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsCMProjectFuncModuleEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsCMProjectFuncModuleWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsCMProjectFuncModuleWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsCMProjectFuncModuleWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsCMProjectFuncModuleWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsCMProjectFuncModuleEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsCMProjectFuncModuleEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",conCMProjectFuncModule.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsCMProjectFuncModuleEN._CurrTabName, strPrjId);
List<clsCMProjectFuncModuleEN> arrCMProjectFuncModuleObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrCMProjectFuncModuleObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsCMProjectFuncModuleEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsCMProjectFuncModuleEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsCMProjectFuncModuleWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsCMProjectFuncModuleWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsCMProjectFuncModuleEN._CurrTabName, strPrjId);
List<clsCMProjectFuncModuleEN> arrCMProjectFuncModuleObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrCMProjectFuncModuleObjLstCache = CacheHelper.Get<List<clsCMProjectFuncModuleEN>>(strKey);
}
else
{
