...
from objvCMFuncParaEN in arrvCMFuncParaObjLstCache
where objvCMFuncParaEN.CmFuncParaId == strCmFuncParaId 
select objvCMFuncParaEN;
if (arrvCMFuncParaObjLst_Sel.Count() == 0)
{
   clsvCMFuncParaEN obj = clsvCMFuncParaWApi.GetObjByCmFuncParaId(strCmFuncParaId);
   if (obj != null)
 {
...
public static IEnumerable<clsvCMFuncParaEN> GetObjLstByCmFuncParaIdLstCache(List<string> arrCmFuncParaId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvCMFuncParaEN._CurrTabName);
List<clsvCMFuncParaEN> arrvCMFuncParaObjLstCache = GetObjLstCache();
IEnumerable <clsvCMFuncParaEN> arrvCMFuncParaObjLst_Sel =
from objvCMFuncParaEN in arrvCMFuncParaObjLstCache
where arrCmFuncParaId.Contains(objvCMFuncParaEN.CmFuncParaId)
select objvCMFuncParaEN;
...
if (clsvCMFuncParaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvCMFuncParaEN._AttributeName)
{
...
var strKey = clsvCMFuncParaEN._CurrTabName;
List<clsvCMFuncParaEN> arrvCMFuncParaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
