...
clsCMFeatureRequirementWApi.ReFreshCache(objCMFeatureRequirementEN.PrjId);
return bolResult;
}
...
from objCMFeatureRequirementEN in arrCMFeatureRequirementObjLstCache
where objCMFeatureRequirementEN.mId == lngmId 
select objCMFeatureRequirementEN;
if (arrCMFeatureRequirementObjLst_Sel.Count() == 0)
{
   clsCMFeatureRequirementEN obj = clsCMFeatureRequirementWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsCMFeatureRequirementEN> GetObjLstByMIdLstCache(List<long> arrMId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsCMFeatureRequirementEN._CurrTabName, strPrjId);
List<clsCMFeatureRequirementEN> arrCMFeatureRequirementObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsCMFeatureRequirementEN> arrCMFeatureRequirementObjLst_Sel =
from objCMFeatureRequirementEN in arrCMFeatureRequirementObjLstCache
where arrMId.Contains(objCMFeatureRequirementEN.mId)
select objCMFeatureRequirementEN;
...
if ((int)jobjReturn0["errorId"] == 0)
{
 clsCMFeatureRequirementEN objCMFeatureRequirementEN = clsCMFeatureRequirementWApi.GetObjBymId(long.Parse(arrmId[0]));
clsCMFeatureRequirementWApi.ReFreshCache(objCMFeatureRequirementEN.PrjId);
var intReturnInt = (int)jobjReturn0["returnInt"];
return intReturnInt;
}
...
if (clsCMFeatureRequirementEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsCMFeatureRequirementEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsCMFeatureRequirementWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsCMFeatureRequirementWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsCMFeatureRequirementWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsCMFeatureRequirementWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsCMFeatureRequirementEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsCMFeatureRequirementEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",conCMFeatureRequirement.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsCMFeatureRequirementEN._CurrTabName, strPrjId);
List<clsCMFeatureRequirementEN> arrCMFeatureRequirementObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrCMFeatureRequirementObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsCMFeatureRequirementEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsCMFeatureRequirementEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsCMFeatureRequirementWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsCMFeatureRequirementWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsCMFeatureRequirementEN._CurrTabName, strPrjId);
List<clsCMFeatureRequirementEN> arrCMFeatureRequirementObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrCMFeatureRequirementObjLstCache = CacheHelper.Get<List<clsCMFeatureRequirementEN>>(strKey);
}
else
{
