...
clsCMFuncParaWApi.ReFreshCache();
return bolResult;
}
...
{
if (strCmFuncParaId == "") return null;
...
{
if (string.IsNullOrEmpty(strCmFuncParaId) == true) return null;
//初始化列表缓存
...
from objCMFuncParaEN in arrCMFuncParaObjLstCache
where objCMFuncParaEN.CmFuncParaId == strCmFuncParaId 
select objCMFuncParaEN;
if (arrCMFuncParaObjLst_Sel.Count() == 0)
{
   clsCMFuncParaEN obj = clsCMFuncParaWApi.GetObjByCmFuncParaId(strCmFuncParaId);
   if (obj != null)
 {
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrCmFuncParaId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsCMFuncParaEN> GetObjLstByCmFuncParaIdLstCache(List<string> arrCmFuncParaId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsCMFuncParaEN._CurrTabName);
List<clsCMFuncParaEN> arrCMFuncParaObjLstCache = GetObjLstCache();
IEnumerable <clsCMFuncParaEN> arrCMFuncParaObjLst_Sel =
from objCMFuncParaEN in arrCMFuncParaObjLstCache
where arrCmFuncParaId.Contains(objCMFuncParaEN.CmFuncParaId)
select objCMFuncParaEN;
...
if (clsCMFuncParaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsCMFuncParaEN._AttributeName)
{
...
var strKey = clsCMFuncParaEN._CurrTabName;
List<clsCMFuncParaEN> arrCMFuncParaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
