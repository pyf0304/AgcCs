...
from objvCMRequirementEN in arrvCMRequirementObjLstCache
where objvCMRequirementEN.ReqId == strReqId 
select objvCMRequirementEN;
if (arrvCMRequirementObjLst_Sel.Count() == 0)
{
   clsvCMRequirementEN obj = clsvCMRequirementWApi.GetObjByReqId(strReqId);
   if (obj != null)
 {
...
public static IEnumerable<clsvCMRequirementEN> GetObjLstByReqIdLstCache(List<string> arrReqId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsvCMRequirementEN._CurrTabName, strPrjId);
List<clsvCMRequirementEN> arrvCMRequirementObjLstCache = GetObjLstCache(strPrjId);
IEnumerable <clsvCMRequirementEN> arrvCMRequirementObjLst_Sel =
from objvCMRequirementEN in arrvCMRequirementObjLstCache
where arrReqId.Contains(objvCMRequirementEN.ReqId)
select objvCMRequirementEN;
...
if (clsvCMRequirementEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvCMRequirementEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvCMRequirementWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvCMRequirementWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvCMRequirementWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvCMRequirementWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
var strWhereCond = "1=1";
if (string.IsNullOrEmpty(clsvCMRequirementEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsvCMRequirementEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",convCMRequirement.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsvCMRequirementEN._CurrTabName, strPrjId);
List<clsvCMRequirementEN> arrvCMRequirementObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
return arrvCMRequirementObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsvCMRequirementEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsvCMRequirementEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsvCMRequirementWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsvCMRequirementWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsvCMRequirementEN._CurrTabName, strPrjId);
List<clsvCMRequirementEN> arrvCMRequirementObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrvCMRequirementObjLstCache = CacheHelper.Get<List<clsvCMRequirementEN>>(strKey);
}
else
{
