 ...
 /*-- -- -- -- -- -- -- -- -- -- --
 类名:clsCMUserProjectWApi
 表名:CMUserProject(00050511)
 * 版本:2025.08.02.1(服务器:PYF-THINKPAD)
 日期:2025/08/09 21:40:30
 生成者:pyf
 生成服务器IP:
 工程名称:AGC(0005)
...
/// 静态类
/// </summary>
public static class  clsCMUserProjectWApi_Static
{
...
try
{
objCMUserProjectEN.sfUpdFldSetStr = objCMUserProjectEN.getsfUpdFldSetStr();
clsCMUserProjectWApi.CheckPropertyNew(objCMUserProjectEN); 
bool bolResult = clsCMUserProjectWApi.UpdateRecord(objCMUserProjectEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCMUserProjectWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsCMUserProjectEN objCMUserProjectEN)
{
//检测记录是否存在
...
public static bool AddNewRecord(this clsCMUserProjectEN objCMUserProjectEN)
{
try
{
clsCMUserProjectWApi.CheckPropertyNew(objCMUserProjectEN); 
bool bolResult = clsCMUserProjectWApi.AddNewRecord(objCMUserProjectEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCMUserProjectWApi.ReFreshCache();
return bolResult;
}
...
{
clsCMUserProjectWApi.CheckPropertyNew(objCMUserProjectEN); 
bool bolResult = clsCMUserProjectWApi.UpdateWithCondition(objCMUserProjectEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCMUserProjectWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsCMUserProjectWApi
{
private static readonly string mstrApiControllerName = "CMUserProjectApi";
...
{
if (!Object.Equals(null, objCMUserProjectEN.UserId) && GetStrLen(objCMUserProjectEN.UserId) > 18)
{
 throw new Exception("字段[用户Id]的长度不能超过18!");
}
if (!Object.Equals(null, objCMUserProjectEN.CmPrjId) && GetStrLen(objCMUserProjectEN.CmPrjId) > 6)
{
 throw new Exception("字段[Cm工程Id]的长度不能超过6!");
}
if (!Object.Equals(null, objCMUserProjectEN.ProjectPath) && GetStrLen(objCMUserProjectEN.ProjectPath) > 500)
{
 throw new Exception("字段[工程路径]的长度不能超过500!");
}
if (!Object.Equals(null, objCMUserProjectEN.UpdDate) && GetStrLen(objCMUserProjectEN.UpdDate) > 20)
{
 throw new Exception("字段[修改日期]的长度不能超过20!");
}
if (!Object.Equals(null, objCMUserProjectEN.UpdUserId) && GetStrLen(objCMUserProjectEN.UpdUserId) > 20)
{
 throw new Exception("字段[修改用户Id]的长度不能超过20!");
}
if (!Object.Equals(null, objCMUserProjectEN.Memo) && GetStrLen(objCMUserProjectEN.Memo) > 1000)
{
...
objCMUserProjectEN = JsonConvert.DeserializeObject<clsCMUserProjectEN>(strJson);
return objCMUserProjectEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsCMUserProjectEN GetFirstObj(string strWhereCond)
{
...
from objCMUserProjectEN in arrCMUserProjectObjLstCache
where objCMUserProjectEN.mId == lngmId 
select objCMUserProjectEN;
if (arrCMUserProjectObjLst_Sel.Count() == 0)
{
   clsCMUserProjectEN obj = clsCMUserProjectWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
{
 List<clsCMUserProjectEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsCMUserProjectEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsCMUserProjectEN> GetObjLstByMIdLst(List<long> arrMId)
{
 List<clsCMUserProjectEN> arrObjLst; 
...
public static IEnumerable<clsCMUserProjectEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsCMUserProjectEN._CurrTabName);
List<clsCMUserProjectEN> arrCMUserProjectObjLstCache = GetObjLstCache();
IEnumerable <clsCMUserProjectEN> arrCMUserProjectObjLst_Sel =
from objCMUserProjectEN in arrCMUserProjectObjLstCache
where arrMId.Contains(objCMUserProjectEN.mId)
select objCMUserProjectEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsCMUserProjectEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsCMUserProjectEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsCMUserProjectEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsCMUserProjectEN> arrObjLst; 
...
try
{
 clsCMUserProjectEN objCMUserProjectEN = clsCMUserProjectWApi.GetObjBymId(lngmId);
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelCMUserProjects(List<string> arrmId)
{
string strAction = "DelCMUserProjects";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrmId);
...
{
string strAction = "DelCMUserProjectsByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsCMUserProjectEN objCMUserProjectEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsCMUserProjectEN>(objCMUserProjectEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <param name = "objCMUserProjectEN">需要添加的表对象</param>
 /// <returns>返回新添加记录的关键字</returns>
public static string AddNewRecordWithReturnKey(clsCMUserProjectEN objCMUserProjectEN)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsCMUserProjectEN objCMUserProjectEN)
{
if (string.IsNullOrEmpty(objCMUserProjectEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objCMUserProjectEN.mId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsCMUserProjectEN objCMUserProjectEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objCMUserProjectEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsCMUserProjectEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsCMUserProjectEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsCMUserProjectEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsCMUserProjectEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsCMUserProjectEN objInFor in arrObj)
{
//Rows
...
if (clsCMUserProjectWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsCMUserProjectEN._CurrTabName);
CacheHelper.Remove(strKey);
clsCMUserProjectWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsCMUserProjectEN._CurrTabName;
List<clsCMUserProjectEN> arrCMUserProjectObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsCMUserProjectEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conCMUserProject.mId, Type.GetType("System.Int64"));
objDT.Columns.Add(conCMUserProject.UserId, Type.GetType("System.String"));
objDT.Columns.Add(conCMUserProject.CmPrjId, Type.GetType("System.String"));
objDT.Columns.Add(conCMUserProject.ProjectPath, Type.GetType("System.String"));
objDT.Columns.Add(conCMUserProject.UpdDate, Type.GetType("System.String"));
objDT.Columns.Add(conCMUserProject.UpdUserId, Type.GetType("System.String"));
objDT.Columns.Add(conCMUserProject.Memo, Type.GetType("System.String"));
foreach (clsCMUserProjectEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conCMUserProject.mId] = objInFor[conCMUserProject.mId];
objDR[conCMUserProject.UserId] = objInFor[conCMUserProject.UserId];
objDR[conCMUserProject.CmPrjId] = objInFor[conCMUserProject.CmPrjId];
objDR[conCMUserProject.ProjectPath] = objInFor[conCMUserProject.ProjectPath];
objDR[conCMUserProject.UpdDate] = objInFor[conCMUserProject.UpdDate];
objDR[conCMUserProject.UpdUserId] = objInFor[conCMUserProject.UpdUserId];
objDR[conCMUserProject.Memo] = objInFor[conCMUserProject.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4CMUserProject : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCMUserProjectWApi.ReFreshThisCache();
}
}
