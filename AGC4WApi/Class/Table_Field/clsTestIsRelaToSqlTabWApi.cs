...
/// 静态类
/// </summary>
public static class  clsTestIsRelaToSqlTabWApi_Static
{
...
try
{
objTestIsRelaToSqlTabEN.sfUpdFldSetStr = objTestIsRelaToSqlTabEN.getsfUpdFldSetStr();
clsTestIsRelaToSqlTabWApi.CheckPropertyNew(objTestIsRelaToSqlTabEN); 
bool bolResult = clsTestIsRelaToSqlTabWApi.UpdateRecord(objTestIsRelaToSqlTabEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTestIsRelaToSqlTabWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN)
{
//检测记录是否存在
...
public static bool AddNewRecord(this clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN)
{
try
{
clsTestIsRelaToSqlTabWApi.CheckPropertyNew(objTestIsRelaToSqlTabEN); 
bool bolResult = clsTestIsRelaToSqlTabWApi.AddNewRecord(objTestIsRelaToSqlTabEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTestIsRelaToSqlTabWApi.ReFreshCache();
return bolResult;
}
...
{
clsTestIsRelaToSqlTabWApi.CheckPropertyNew(objTestIsRelaToSqlTabEN); 
bool bolResult = clsTestIsRelaToSqlTabWApi.UpdateWithCondition(objTestIsRelaToSqlTabEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTestIsRelaToSqlTabWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsTestIsRelaToSqlTabWApi
{
private static readonly string mstrApiControllerName = "TestIsRelaToSqlTabApi";
...
public static clsCommFun4BL objCommFun4WApi = null;
 ...
 public clsTestIsRelaToSqlTabWApi()
 {
 }
...
{
if (!Object.Equals(null, objTestIsRelaToSqlTabEN.UserId) && GetStrLen(objTestIsRelaToSqlTabEN.UserId) > 18)
{
 throw new Exception("字段[用户Id]的长度不能超过18!");
}
if (!Object.Equals(null, objTestIsRelaToSqlTabEN.TabId) && GetStrLen(objTestIsRelaToSqlTabEN.TabId) > 8)
{
...
clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN;
...
objTestIsRelaToSqlTabEN = JsonConvert.DeserializeObject<clsTestIsRelaToSqlTabEN>(strJson);
return objTestIsRelaToSqlTabEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsTestIsRelaToSqlTabEN GetFirstObj(string strWhereCond)
{
...
from objTestIsRelaToSqlTabEN in arrTestIsRelaToSqlTabObjLstCache
where objTestIsRelaToSqlTabEN.mId == lngmId 
select objTestIsRelaToSqlTabEN;
if (arrTestIsRelaToSqlTabObjLst_Sel.Count() == 0)
{
   clsTestIsRelaToSqlTabEN obj = clsTestIsRelaToSqlTabWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
{
 List<clsTestIsRelaToSqlTabEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsTestIsRelaToSqlTabEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsTestIsRelaToSqlTabEN> GetObjLstByMIdLst(List<long> arrMId)
{
 List<clsTestIsRelaToSqlTabEN> arrObjLst; 
...
public static IEnumerable<clsTestIsRelaToSqlTabEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsTestIsRelaToSqlTabEN._CurrTabName);
List<clsTestIsRelaToSqlTabEN> arrTestIsRelaToSqlTabObjLstCache = GetObjLstCache();
IEnumerable <clsTestIsRelaToSqlTabEN> arrTestIsRelaToSqlTabObjLst_Sel =
from objTestIsRelaToSqlTabEN in arrTestIsRelaToSqlTabObjLstCache
where arrMId.Contains(objTestIsRelaToSqlTabEN.mId)
select objTestIsRelaToSqlTabEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsTestIsRelaToSqlTabEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsTestIsRelaToSqlTabEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsTestIsRelaToSqlTabEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsTestIsRelaToSqlTabEN> arrObjLst; 
...
try
{
 clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN = clsTestIsRelaToSqlTabWApi.GetObjBymId(lngmId);
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelTestIsRelaToSqlTabs(List<string> arrmId)
{
string strAction = "DelTestIsRelaToSqlTabs";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrmId);
...
{
string strAction = "DelTestIsRelaToSqlTabsByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsTestIsRelaToSqlTabEN>(objTestIsRelaToSqlTabEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithReturnKey(clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN)
{
if (string.IsNullOrEmpty(objTestIsRelaToSqlTabEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objTestIsRelaToSqlTabEN.mId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsTestIsRelaToSqlTabEN objTestIsRelaToSqlTabEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objTestIsRelaToSqlTabEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsTestIsRelaToSqlTabEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsTestIsRelaToSqlTabEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsTestIsRelaToSqlTabEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsTestIsRelaToSqlTabEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsTestIsRelaToSqlTabEN objInFor in arrObj)
{
//Rows
...
if (clsTestIsRelaToSqlTabWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsTestIsRelaToSqlTabEN._CurrTabName);
CacheHelper.Remove(strKey);
clsTestIsRelaToSqlTabWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsTestIsRelaToSqlTabEN._CurrTabName;
List<clsTestIsRelaToSqlTabEN> arrTestIsRelaToSqlTabObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsTestIsRelaToSqlTabEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conTestIsRelaToSqlTab.mId, Type.GetType("System.Int64"));
objDT.Columns.Add(conTestIsRelaToSqlTab.UserId, Type.GetType("System.String"));
objDT.Columns.Add(conTestIsRelaToSqlTab.TabId, Type.GetType("System.String"));
foreach (clsTestIsRelaToSqlTabEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conTestIsRelaToSqlTab.mId] = objInFor[conTestIsRelaToSqlTab.mId];
objDR[conTestIsRelaToSqlTab.UserId] = objInFor[conTestIsRelaToSqlTab.UserId];
objDR[conTestIsRelaToSqlTab.TabId] = objInFor[conTestIsRelaToSqlTab.TabId];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4TestIsRelaToSqlTab : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTestIsRelaToSqlTabWApi.ReFreshThisCache();
}
}
