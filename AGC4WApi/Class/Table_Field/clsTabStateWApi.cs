...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsTabStateEN objTabStateEN)
{
//检测记录是否存在
...
try
{
 if (string.IsNullOrEmpty(objTabStateEN.TabStateId) == true || clsTabStateWApi.IsExist(objTabStateEN.TabStateId) == true)
 {
     objTabStateEN.TabStateId = clsTabStateWApi.GetMaxStrId();
 }
clsTabStateWApi.CheckPropertyNew(objTabStateEN); 
...
{
if (string.IsNullOrEmpty(strTabStateId) == true) return null;
//初始化列表缓存
...
from objTabStateEN in arrTabStateObjLstCache
where objTabStateEN.TabStateId == strTabStateId 
select objTabStateEN;
if (arrTabStateObjLst_Sel.Count() == 0)
{
   clsTabStateEN obj = clsTabStateWApi.GetObjByTabStateId(strTabStateId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsTabStateEN> arrTabStateObjLstCache = GetObjLstCache();
IEnumerable <clsTabStateEN> arrTabStateObjLst_Sel1 =
from objTabStateEN in arrTabStateObjLstCache
where objTabStateEN.TabStateId == strTabStateId 
select objTabStateEN;
List <clsTabStateEN> arrTabStateObjLst_Sel = new List<clsTabStateEN>();
foreach (clsTabStateEN obj in arrTabStateObjLst_Sel1)
{
arrTabStateObjLst_Sel.Add(obj);
}
if (arrTabStateObjLst_Sel.Count > 0)
{
return arrTabStateObjLst_Sel[0].TabStateName;
}
string strErrMsgForGetObjById = string.Format("在TabState对象缓存列表中,找不到记录[TabStateId = {0}](函数:{1})", strTabStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsTabStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在TabState对象缓存列表中,找不到记录的相关名称[TabStateId = {0}](函数:{1})", strTabStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsTabStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrTabStateId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsTabStateEN> GetObjLstByTabStateIdLstCache(List<string> arrTabStateId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsTabStateEN._CurrTabName);
List<clsTabStateEN> arrTabStateObjLstCache = GetObjLstCache();
IEnumerable <clsTabStateEN> arrTabStateObjLst_Sel =
from objTabStateEN in arrTabStateObjLstCache
where arrTabStateId.Contains(objTabStateEN.TabStateId)
select objTabStateEN;
...
if (clsTabStateEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsTabStateEN._AttributeName)
{
...
var strKey = clsTabStateEN._CurrTabName;
List<clsTabStateEN> arrTabStateObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
