...
/// 静态类
/// </summary>
public static class  clsPrimaryTypeWApi_Static
{
...
return objPrimaryTypeEN;
 ...
 /// </summary>
 /// <param name = "objPrimaryTypeEN">需要设置字段值的实体对象</param>
 /// <param name = "strPrimaryTypeENName">主键类型英文名</param>
 /// <param name = "strComparisonOp">比较运算符,如果有值,可用于组织条件串</param>
 /// <returns>返回对象,可以继续连写</returns>
...
try
{
objPrimaryTypeEN.sfUpdFldSetStr = objPrimaryTypeEN.getsfUpdFldSetStr();
clsPrimaryTypeWApi.CheckPropertyNew(objPrimaryTypeEN); 
bool bolResult = clsPrimaryTypeWApi.UpdateRecord(objPrimaryTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrimaryTypeWApi.ReFreshCache();
return bolResult;
}
...
public static bool AddNewRecord(this clsPrimaryTypeEN objPrimaryTypeEN)
{
 if (string.IsNullOrEmpty(objPrimaryTypeEN.PrimaryTypeId) == true)
 {
...
clsStackTrace.GetCurrClassFunction()); 
 ...
 if (clsPrimaryTypeWApi.IsExist(objPrimaryTypeEN.PrimaryTypeId) == true)
 {
...
{
clsPrimaryTypeWApi.CheckPropertyNew(objPrimaryTypeEN); 
bool bolResult = clsPrimaryTypeWApi.AddNewRecord(objPrimaryTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrimaryTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objPrimaryTypeEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsPrimaryTypeEN objPrimaryTypeEN)
{
try
{
clsPrimaryTypeWApi.CheckPropertyNew(objPrimaryTypeEN); 
string strPrimaryTypeId = clsPrimaryTypeWApi.AddNewRecordWithMaxId(objPrimaryTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrimaryTypeWApi.ReFreshCache();
...
{
clsPrimaryTypeWApi.CheckPropertyNew(objPrimaryTypeEN); 
bool bolResult = clsPrimaryTypeWApi.UpdateWithCondition(objPrimaryTypeEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrimaryTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class enumPrimaryTypeWA
{
 /// <summary>
 /// 非关键字
 /// </summary>
...
public const string CompositePrimaryKey_07 = "07";
 ...
 /// </summary>
public class clsPrimaryTypeWApi
{
private static readonly string mstrApiControllerName = "PrimaryTypeApi";
...
List<clsPrimaryTypeEN> arrObjLst = GetObjLst("1=1");
...
List<clsPrimaryTypeEN> arrObjLst = clsPrimaryTypeWApi.GetObjLst(strCondition).OrderBy(x=>x.OrderNum).ToList();
//初始化一个对象列表
...
{
if (!Object.Equals(null, objPrimaryTypeEN.PrimaryTypeId) && GetStrLen(objPrimaryTypeEN.PrimaryTypeId) > 2)
{
 throw new Exception("字段[主键类型ID]的长度不能超过2!");
}
if (!Object.Equals(null, objPrimaryTypeEN.PrimaryTypeName) && GetStrLen(objPrimaryTypeEN.PrimaryTypeName) > 30)
{
 throw new Exception("字段[主键类型名]的长度不能超过30!");
}
if (!Object.Equals(null, objPrimaryTypeEN.PrimaryTypeENName) && GetStrLen(objPrimaryTypeEN.PrimaryTypeENName) > 50)
{
 throw new Exception("字段[主键类型英文名]的长度不能超过50!");
}
if (!Object.Equals(null, objPrimaryTypeEN.Memo) && GetStrLen(objPrimaryTypeEN.Memo) > 1000)
{
...
{
if (strPrimaryTypeId == "") return null;
string strAction = "GetObjByPrimaryTypeId";
clsPrimaryTypeEN objPrimaryTypeEN;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strPrimaryTypeId"] = strPrimaryTypeId,
};
try
{
...
objPrimaryTypeEN = JsonConvert.DeserializeObject<clsPrimaryTypeEN>(strJson);
return objPrimaryTypeEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsPrimaryTypeEN GetFirstObj(string strWhereCond)
{
...
{
if (string.IsNullOrEmpty(strPrimaryTypeId) == true) return null;
//初始化列表缓存
...
from objPrimaryTypeEN in arrPrimaryTypeObjLstCache
where objPrimaryTypeEN.PrimaryTypeId == strPrimaryTypeId 
select objPrimaryTypeEN;
if (arrPrimaryTypeObjLst_Sel.Count() == 0)
{
   clsPrimaryTypeEN obj = clsPrimaryTypeWApi.GetObjByPrimaryTypeId(strPrimaryTypeId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsPrimaryTypeEN> arrPrimaryTypeObjLstCache = GetObjLstCache();
IEnumerable <clsPrimaryTypeEN> arrPrimaryTypeObjLst_Sel1 =
from objPrimaryTypeEN in arrPrimaryTypeObjLstCache
where objPrimaryTypeEN.PrimaryTypeId == strPrimaryTypeId 
select objPrimaryTypeEN;
List <clsPrimaryTypeEN> arrPrimaryTypeObjLst_Sel = new List<clsPrimaryTypeEN>();
foreach (clsPrimaryTypeEN obj in arrPrimaryTypeObjLst_Sel1)
{
arrPrimaryTypeObjLst_Sel.Add(obj);
}
if (arrPrimaryTypeObjLst_Sel.Count > 0)
{
return arrPrimaryTypeObjLst_Sel[0].PrimaryTypeName;
}
string strErrMsgForGetObjById = string.Format("在PrimaryType对象缓存列表中,找不到记录[PrimaryTypeId = {0}](函数:{1})", strPrimaryTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsPrimaryTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在PrimaryType对象缓存列表中,找不到记录的相关名称[PrimaryTypeId = {0}](函数:{1})", strPrimaryTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsPrimaryTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
{
 List<clsPrimaryTypeEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsPrimaryTypeEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsPrimaryTypeEN> GetObjLstByPrimaryTypeIdLst(List<string> arrPrimaryTypeId)
{
 List<clsPrimaryTypeEN> arrObjLst; 
string strAction = "GetObjLstByPrimaryTypeIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrPrimaryTypeId);
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrPrimaryTypeId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsPrimaryTypeEN> GetObjLstByPrimaryTypeIdLstCache(List<string> arrPrimaryTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsPrimaryTypeEN._CurrTabName);
List<clsPrimaryTypeEN> arrPrimaryTypeObjLstCache = GetObjLstCache();
IEnumerable <clsPrimaryTypeEN> arrPrimaryTypeObjLst_Sel =
from objPrimaryTypeEN in arrPrimaryTypeObjLstCache
where arrPrimaryTypeId.Contains(objPrimaryTypeEN.PrimaryTypeId)
select objPrimaryTypeEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsPrimaryTypeEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsPrimaryTypeEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsPrimaryTypeEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsPrimaryTypeEN> arrObjLst; 
...
try
{
 clsPrimaryTypeEN objPrimaryTypeEN = clsPrimaryTypeWApi.GetObjByPrimaryTypeId(strPrimaryTypeId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strPrimaryTypeId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelPrimaryTypes(List<string> arrPrimaryTypeId)
{
string strAction = "DelPrimaryTypes";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrPrimaryTypeId);
...
{
string strAction = "DelPrimaryTypesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsPrimaryTypeEN objPrimaryTypeEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsPrimaryTypeEN>(objPrimaryTypeEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsPrimaryTypeEN objPrimaryTypeEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrimaryTypeWApi.ReFreshCache();
var strPrimaryTypeId = (string)jobjReturn0["returnStr"];
return strPrimaryTypeId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsPrimaryTypeEN objPrimaryTypeEN)
{
if (string.IsNullOrEmpty(objPrimaryTypeEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objPrimaryTypeEN.PrimaryTypeId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsPrimaryTypeEN objPrimaryTypeEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objPrimaryTypeEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsPrimaryTypeEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsPrimaryTypeEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsPrimaryTypeEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsPrimaryTypeEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsPrimaryTypeEN objInFor in arrObj)
{
//Rows
...
if (clsPrimaryTypeWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsPrimaryTypeEN._CurrTabName);
CacheHelper.Remove(strKey);
clsPrimaryTypeWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsPrimaryTypeEN._CurrTabName;
List<clsPrimaryTypeEN> arrPrimaryTypeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsPrimaryTypeEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conPrimaryType.PrimaryTypeId, Type.GetType("System.String"));
objDT.Columns.Add(conPrimaryType.PrimaryTypeName, Type.GetType("System.String"));
objDT.Columns.Add(conPrimaryType.PrimaryTypeENName, Type.GetType("System.String"));
objDT.Columns.Add(conPrimaryType.OrderNum, Type.GetType("System.Int32"));
objDT.Columns.Add(conPrimaryType.Memo, Type.GetType("System.String"));
foreach (clsPrimaryTypeEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conPrimaryType.PrimaryTypeId] = objInFor[conPrimaryType.PrimaryTypeId];
objDR[conPrimaryType.PrimaryTypeName] = objInFor[conPrimaryType.PrimaryTypeName];
objDR[conPrimaryType.PrimaryTypeENName] = objInFor[conPrimaryType.PrimaryTypeENName];
objDR[conPrimaryType.OrderNum] = objInFor[conPrimaryType.OrderNum];
objDR[conPrimaryType.Memo] = objInFor[conPrimaryType.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4PrimaryType : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrimaryTypeWApi.ReFreshThisCache();
}
}
