...
clsPrjTabFldWApi.ReFreshCache(objPrjTabFldEN.PrjId);
return bolResult;
}
...
public static IEnumerable<clsPrjTabFldEN> GetObjLstByMIdLstCache(List<long> arrMId, string strPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsPrjTabFldEN._CurrTabName, strPrjId);
List<clsPrjTabFldEN> arrPrjTabFldObjLstCache = GetObjLstCache(strPrjId);
...
if (clsPrjTabFldEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsPrjTabFldEN._AttributeName)
{
...
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsPrjTabFldWApi.ReFreshThisCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsPrjTabFldWApi.ReFreshThisCache)", strPrjId.Length);
throw new Exception (strMsg);
}
string strMsg0;
...
string strKey = string.Format("{0}_{1}", clsPrjTabFldEN._CurrTabName, strPrjId);
CacheHelper.Remove(strKey);
clsPrjTabFldWApi.objCommFun4WApi.ReFreshCache(strPrjId.ToString());
}
}
...
if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsPrjTabFldWApi.GetObjLstCache)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsPrjTabFldWApi.GetObjLstCache)", strPrjId.Length);
throw new Exception (strMsg);
}
//初始化列表缓存
...
{
strWhereCond =string.Format(clsPrjTabFldEN._WhereFormat, strPrjId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",conPrjTabFld.PrjId, strPrjId);
}
var strKey = string.Format("{0}_{1}", clsPrjTabFldEN._CurrTabName, strPrjId);
...
return arrPrjTabFldObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsPrjTabFldEN> GetObjLstCacheFromObjLst(string strPrjId,List<clsPrjTabFldEN> arrObjLst_P)
{


if (string.IsNullOrEmpty(strPrjId) == true)
{
  var strMsg = string.Format("参数:[strPrjId]不能为空！(In clsPrjTabFldWApi.GetObjLstCacheFromObjLst)");
 throw new Exception  (strMsg);
}
if (strPrjId.Length != 4)
{
var strMsg = string.Format("缓存分类变量:[strPrjId]的长度:[{0}]不正确！(clsPrjTabFldWApi.GetObjLstCacheFromObjLst)", strPrjId.Length);
throw new Exception (strMsg);
}
var strKey = string.Format("{0}_{1}", clsPrjTabFldEN._CurrTabName, strPrjId);
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPrjTabFldWApi.ReFreshThisCache(strPrjId);
}
}
