...
/// 静态类
/// </summary>
public static class  clsFldStateWApi_Static
{
...
return objFldStateEN;
 ...
 /// </summary>
 /// <param name = "objFldStateEN">需要设置字段值的实体对象</param>
 /// <param name = "strFldStateName">字段状态名</param>
 /// <param name = "strComparisonOp">比较运算符,如果有值,可用于组织条件串</param>
 /// <returns>返回对象,可以继续连写</returns>
...
try
{
objFldStateEN.sfUpdFldSetStr = objFldStateEN.getsfUpdFldSetStr();
clsFldStateWApi.CheckPropertyNew(objFldStateEN); 
bool bolResult = clsFldStateWApi.UpdateRecord(objFldStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFldStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsFldStateEN objFldStateEN)
{
//检测记录是否存在
...
public static bool AddNewRecord(this clsFldStateEN objFldStateEN)
{
try
{
 if (string.IsNullOrEmpty(objFldStateEN.FldStateId) == true || clsFldStateWApi.IsExist(objFldStateEN.FldStateId) == true)
 {
     objFldStateEN.FldStateId = clsFldStateWApi.GetMaxStrId();
 }
clsFldStateWApi.CheckPropertyNew(objFldStateEN); 
bool bolResult = clsFldStateWApi.AddNewRecord(objFldStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFldStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objFldStateEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsFldStateEN objFldStateEN)
{
try
{
clsFldStateWApi.CheckPropertyNew(objFldStateEN); 
string strFldStateId = clsFldStateWApi.AddNewRecordWithMaxId(objFldStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFldStateWApi.ReFreshCache();
...
{
clsFldStateWApi.CheckPropertyNew(objFldStateEN); 
bool bolResult = clsFldStateWApi.UpdateWithCondition(objFldStateEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFldStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsFldStateWApi
{
private static readonly string mstrApiControllerName = "FldStateApi";
...
List<clsFldStateEN> arrObjLst = GetObjLst("1=1");
...
List<clsFldStateEN> arrObjLst = clsFldStateWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
if (!Object.Equals(null, objFldStateEN.FldStateId) && GetStrLen(objFldStateEN.FldStateId) > 2)
{
 throw new Exception("字段[字段状态Id]的长度不能超过2!");
}
if (!Object.Equals(null, objFldStateEN.FldStateName) && GetStrLen(objFldStateEN.FldStateName) > 20)
{
 throw new Exception("字段[字段状态名]的长度不能超过20!");
}
if (!Object.Equals(null, objFldStateEN.Memo) && GetStrLen(objFldStateEN.Memo) > 1000)
{
...
{
if (strFldStateId == "") return null;
string strAction = "GetObjByFldStateId";
clsFldStateEN objFldStateEN;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strFldStateId"] = strFldStateId,
};
try
{
...
objFldStateEN = JsonConvert.DeserializeObject<clsFldStateEN>(strJson);
return objFldStateEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsFldStateEN GetFirstObj(string strWhereCond)
{
...
{
if (string.IsNullOrEmpty(strFldStateId) == true) return null;
//初始化列表缓存
...
from objFldStateEN in arrFldStateObjLstCache
where objFldStateEN.FldStateId == strFldStateId 
select objFldStateEN;
if (arrFldStateObjLst_Sel.Count() == 0)
{
   clsFldStateEN obj = clsFldStateWApi.GetObjByFldStateId(strFldStateId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsFldStateEN> arrFldStateObjLstCache = GetObjLstCache();
IEnumerable <clsFldStateEN> arrFldStateObjLst_Sel1 =
from objFldStateEN in arrFldStateObjLstCache
where objFldStateEN.FldStateId == strFldStateId 
select objFldStateEN;
List <clsFldStateEN> arrFldStateObjLst_Sel = new List<clsFldStateEN>();
foreach (clsFldStateEN obj in arrFldStateObjLst_Sel1)
{
arrFldStateObjLst_Sel.Add(obj);
}
if (arrFldStateObjLst_Sel.Count > 0)
{
return arrFldStateObjLst_Sel[0].FldStateName;
}
string strErrMsgForGetObjById = string.Format("在FldState对象缓存列表中,找不到记录[FldStateId = {0}](函数:{1})", strFldStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsFldStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在FldState对象缓存列表中,找不到记录的相关名称[FldStateId = {0}](函数:{1})", strFldStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsFldStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
arrObjLst = JsonConvert.DeserializeObject<List<clsFldStateEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrFldStateId">关键字列表</param>
 /// <returns>返回对象列表</returns>
public static List<clsFldStateEN> GetObjLstByFldStateIdLst(List<string> arrFldStateId)
{
 List<clsFldStateEN> arrObjLst; 
string strAction = "GetObjLstByFldStateIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrFldStateId);
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <param name = "arrFldStateId">所给的关键字列表</param>
 /// <returns>根据关键字列表获取的对象</returns>
public static IEnumerable<clsFldStateEN> GetObjLstByFldStateIdLstCache(List<string> arrFldStateId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsFldStateEN._CurrTabName);
List<clsFldStateEN> arrFldStateObjLstCache = GetObjLstCache();
IEnumerable <clsFldStateEN> arrFldStateObjLst_Sel =
from objFldStateEN in arrFldStateObjLstCache
where arrFldStateId.Contains(objFldStateEN.FldStateId)
select objFldStateEN;
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsFldStateEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsFldStateEN> arrObjLst; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsFldStateEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsFldStateEN> arrObjLst; 
...
try
{
 clsFldStateEN objFldStateEN = clsFldStateWApi.GetObjByFldStateId(strFldStateId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strFldStateId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelFldStates(List<string> arrFldStateId)
{
string strAction = "DelFldStates";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrFldStateId);
...
{
string strAction = "DelFldStatesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsFldStateEN objFldStateEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsFldStateEN>(objFldStateEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsFldStateEN objFldStateEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFldStateWApi.ReFreshCache();
var strFldStateId = (string)jobjReturn0["returnStr"];
return strFldStateId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsFldStateEN objFldStateEN)
{
if (string.IsNullOrEmpty(objFldStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objFldStateEN.FldStateId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsFldStateEN objFldStateEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objFldStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsFldStateEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsFldStateEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsFldStateEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsFldStateEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsFldStateEN objInFor in arrObj)
{
//Rows
...
if (clsFldStateWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsFldStateEN._CurrTabName);
CacheHelper.Remove(strKey);
clsFldStateWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsFldStateEN._CurrTabName;
List<clsFldStateEN> arrFldStateObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsFldStateEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conFldState.FldStateId, Type.GetType("System.String"));
objDT.Columns.Add(conFldState.FldStateName, Type.GetType("System.String"));
objDT.Columns.Add(conFldState.Memo, Type.GetType("System.String"));
foreach (clsFldStateEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conFldState.FldStateId] = objInFor[conFldState.FldStateId];
objDR[conFldState.FldStateName] = objInFor[conFldState.FldStateName];
objDR[conFldState.Memo] = objInFor[conFldState.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4FldState : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFldStateWApi.ReFreshThisCache();
}
}
