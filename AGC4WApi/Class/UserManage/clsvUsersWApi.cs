...
from objvUsersEN in arrvUsersObjLstCache
...
public static IEnumerable<clsvUsersEN> GetObjLstByUserIdLstCache(List<string> arrUserId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvUsersEN._CurrTabName);
List<clsvUsersEN> arrvUsersObjLstCache = GetObjLstCache();
...
if (clsvUsersEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvUsersEN._AttributeName)
{
...
var strKey = clsvUsersEN._CurrTabName;
List<clsvUsersEN> arrvUsersObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
