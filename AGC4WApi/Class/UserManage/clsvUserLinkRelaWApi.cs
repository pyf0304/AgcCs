...
from objvUserLinkRelaEN in arrvUserLinkRelaObjLstCache
where objvUserLinkRelaEN.mId == lngmId 
select objvUserLinkRelaEN;
if (arrvUserLinkRelaObjLst_Sel.Count() == 0)
{
   clsvUserLinkRelaEN obj = clsvUserLinkRelaWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvUserLinkRelaEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvUserLinkRelaEN._CurrTabName);
List<clsvUserLinkRelaEN> arrvUserLinkRelaObjLstCache = GetObjLstCache();
IEnumerable <clsvUserLinkRelaEN> arrvUserLinkRelaObjLst_Sel =
from objvUserLinkRelaEN in arrvUserLinkRelaObjLstCache
where arrMId.Contains(objvUserLinkRelaEN.mId)
select objvUserLinkRelaEN;
...
if (clsvUserLinkRelaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvUserLinkRelaEN._AttributeName)
{
...
var strKey = clsvUserLinkRelaEN._CurrTabName;
List<clsvUserLinkRelaEN> arrvUserLinkRelaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
