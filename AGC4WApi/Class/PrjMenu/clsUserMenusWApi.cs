...
/// 静态类
/// </summary>
public static class  clsUserMenusWApi_Static
{
...
return objUserMenusEN;
 ...
 /// </summary>
 /// <param name = "objUserMenusEN">需要设置字段值的实体对象</param>
 /// <param name = "bolIsUseRoleMenu">IsUseRoleMenu</param>
 /// <param name = "strComparisonOp">比较运算符,如果有值,可用于组织条件串</param>
 /// <returns>返回对象,可以继续连写</returns>
...
try
{
objUserMenusEN.sfUpdFldSetStr = objUserMenusEN.getsfUpdFldSetStr();
clsUserMenusWApi.CheckPropertyNew(objUserMenusEN); 
bool bolResult = clsUserMenusWApi.UpdateRecord(objUserMenusEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUserMenusWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsUserMenusEN objUserMenusEN)
{
//检测记录是否存在
...
{
clsUserMenusWApi.CheckPropertyNew(objUserMenusEN); 
bool bolResult = clsUserMenusWApi.AddNewRecord(objUserMenusEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUserMenusWApi.ReFreshCache();
return bolResult;
}
...
{
clsUserMenusWApi.CheckPropertyNew(objUserMenusEN); 
bool bolResult = clsUserMenusWApi.UpdateWithCondition(objUserMenusEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUserMenusWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsUserMenusWApi
{
private static readonly string mstrApiControllerName = "UserMenusApi";
...
{
if (!Object.Equals(null, objUserMenusEN.UserId) && GetStrLen(objUserMenusEN.UserId) > 18)
{
 throw new Exception("字段[用户Id]的长度不能超过18!");
}
if (!Object.Equals(null, objUserMenusEN.PrjId) && GetStrLen(objUserMenusEN.PrjId) > 4)
{
 throw new Exception("字段[工程Id]的长度不能超过4!");
}
if (!Object.Equals(null, objUserMenusEN.Memo) && GetStrLen(objUserMenusEN.Memo) > 1000)
{
...
objUserMenusEN = JsonConvert.DeserializeObject<clsUserMenusEN>(strJson);
return objUserMenusEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsUserMenusEN GetFirstObj(string strWhereCond)
{
...
from objUserMenusEN in arrUserMenusObjLstCache
where objUserMenusEN.mId == lngmId 
select objUserMenusEN;
if (arrUserMenusObjLst_Sel.Count() == 0)
{
   clsUserMenusEN obj = clsUserMenusWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
arrObjLst = JsonConvert.DeserializeObject<List<clsUserMenusEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsUserMenusEN> GetObjLstByMIdLst(List<long> arrMId)
{
 List<clsUserMenusEN> arrObjLst; 
...
public static IEnumerable<clsUserMenusEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsUserMenusEN._CurrTabName);
List<clsUserMenusEN> arrUserMenusObjLstCache = GetObjLstCache();
IEnumerable <clsUserMenusEN> arrUserMenusObjLst_Sel =
from objUserMenusEN in arrUserMenusObjLstCache
where arrMId.Contains(objUserMenusEN.mId)
select objUserMenusEN;
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsUserMenusEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsUserMenusEN> arrObjLst; 
...
try
{
 clsUserMenusEN objUserMenusEN = clsUserMenusWApi.GetObjBymId(lngmId);
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelUserMenuss(List<string> arrmId)
{
string strAction = "DelUserMenuss";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrmId);
...
{
string strAction = "DelUserMenussByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsUserMenusEN objUserMenusEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsUserMenusEN>(objUserMenusEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <param name = "objUserMenusEN">需要添加的表对象</param>
 /// <returns>返回新添加记录的关键字</returns>
public static string AddNewRecordWithReturnKey(clsUserMenusEN objUserMenusEN)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsUserMenusEN objUserMenusEN)
{
if (string.IsNullOrEmpty(objUserMenusEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objUserMenusEN.mId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsUserMenusEN objUserMenusEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objUserMenusEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsUserMenusEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsUserMenusEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsUserMenusEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsUserMenusEN._AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsUserMenusEN objInFor in arrObj)
{
//Rows
...
if (clsUserMenusWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsUserMenusEN._CurrTabName);
CacheHelper.Remove(strKey);
clsUserMenusWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsUserMenusEN._CurrTabName;
List<clsUserMenusEN> arrUserMenusObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsUserMenusEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conUserMenus.mId, Type.GetType("System.Int64"));
objDT.Columns.Add(conUserMenus.UserId, Type.GetType("System.String"));
objDT.Columns.Add(conUserMenus.PrjId, Type.GetType("System.String"));
objDT.Columns.Add(conUserMenus.IsUseRoleMenu, Type.GetType("System.Boolean"));
objDT.Columns.Add(conUserMenus.Memo, Type.GetType("System.String"));
foreach (clsUserMenusEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conUserMenus.mId] = objInFor[conUserMenus.mId];
objDR[conUserMenus.UserId] = objInFor[conUserMenus.UserId];
objDR[conUserMenus.PrjId] = objInFor[conUserMenus.PrjId];
objDR[conUserMenus.IsUseRoleMenu] = objInFor[conUserMenus.IsUseRoleMenu];
objDR[conUserMenus.Memo] = objInFor[conUserMenus.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4UserMenus : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsUserMenusWApi.ReFreshThisCache();
}
}
