...
from objvwf_StepPointRelaEN in arrvwf_StepPointRelaObjLstCache
where objvwf_StepPointRelaEN.WorkFlowStepPointRelaId == lngWorkFlowStepPointRelaId 
select objvwf_StepPointRelaEN;
if (arrvwf_StepPointRelaObjLst_Sel.Count() == 0)
{
   clsvwf_StepPointRelaEN obj = clsvwf_StepPointRelaWApi.GetObjByWorkFlowStepPointRelaId(lngWorkFlowStepPointRelaId);
   if (obj != null)
 {
...
public static IEnumerable<clsvwf_StepPointRelaEN> GetObjLstByWorkFlowStepPointRelaIdLstCache(List<long> arrWorkFlowStepPointRelaId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvwf_StepPointRelaEN._CurrTabName);
List<clsvwf_StepPointRelaEN> arrvwf_StepPointRelaObjLstCache = GetObjLstCache();
IEnumerable <clsvwf_StepPointRelaEN> arrvwf_StepPointRelaObjLst_Sel =
from objvwf_StepPointRelaEN in arrvwf_StepPointRelaObjLstCache
where arrWorkFlowStepPointRelaId.Contains(objvwf_StepPointRelaEN.WorkFlowStepPointRelaId)
select objvwf_StepPointRelaEN;
...
if (clsvwf_StepPointRelaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvwf_StepPointRelaEN._AttributeName)
{
...
var strKey = clsvwf_StepPointRelaEN._CurrTabName;
List<clsvwf_StepPointRelaEN> arrvwf_StepPointRelaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
