...
clsUserWebSrvSetWApi.ReFreshCache();
return bolResult;
}
...
from objUserWebSrvSetEN in arrUserWebSrvSetObjLstCache
where objUserWebSrvSetEN.mId == lngmId 
select objUserWebSrvSetEN;
if (arrUserWebSrvSetObjLst_Sel.Count() == 0)
{
   clsUserWebSrvSetEN obj = clsUserWebSrvSetWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsUserWebSrvSetEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsUserWebSrvSetEN._CurrTabName);
List<clsUserWebSrvSetEN> arrUserWebSrvSetObjLstCache = GetObjLstCache();
IEnumerable <clsUserWebSrvSetEN> arrUserWebSrvSetObjLst_Sel =
from objUserWebSrvSetEN in arrUserWebSrvSetObjLstCache
where arrMId.Contains(objUserWebSrvSetEN.mId)
select objUserWebSrvSetEN;
...
if (clsUserWebSrvSetEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsUserWebSrvSetEN._AttributeName)
{
...
var strKey = clsUserWebSrvSetEN._CurrTabName;
List<clsUserWebSrvSetEN> arrUserWebSrvSetObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
