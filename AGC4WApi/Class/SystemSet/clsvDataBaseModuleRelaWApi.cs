...
from objvDataBaseModuleRelaEN in arrvDataBaseModuleRelaObjLstCache
where objvDataBaseModuleRelaEN.mId == lngmId 
select objvDataBaseModuleRelaEN;
if (arrvDataBaseModuleRelaObjLst_Sel.Count() == 0)
{
   clsvDataBaseModuleRelaEN obj = clsvDataBaseModuleRelaWApi.GetObjBymId(lngmId);
   if (obj != null)
 {
...
public static IEnumerable<clsvDataBaseModuleRelaEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvDataBaseModuleRelaEN._CurrTabName);
List<clsvDataBaseModuleRelaEN> arrvDataBaseModuleRelaObjLstCache = GetObjLstCache();
IEnumerable <clsvDataBaseModuleRelaEN> arrvDataBaseModuleRelaObjLst_Sel =
from objvDataBaseModuleRelaEN in arrvDataBaseModuleRelaObjLstCache
where arrMId.Contains(objvDataBaseModuleRelaEN.mId)
select objvDataBaseModuleRelaEN;
...
if (clsvDataBaseModuleRelaEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvDataBaseModuleRelaEN._AttributeName)
{
...
var strKey = clsvDataBaseModuleRelaEN._CurrTabName;
List<clsvDataBaseModuleRelaEN> arrvDataBaseModuleRelaObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
