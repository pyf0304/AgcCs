...
from objvSetFieldVisibilityEN in arrvSetFieldVisibilityObjLstCache
...
public static IEnumerable<clsvSetFieldVisibilityEN> GetObjLstByMIdLstCache(List<long> arrMId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsvSetFieldVisibilityEN._CurrTabName);
List<clsvSetFieldVisibilityEN> arrvSetFieldVisibilityObjLstCache = GetObjLstCache();
...
if (clsvSetFieldVisibilityEN._AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsvSetFieldVisibilityEN._AttributeName)
{
...
var strKey = clsvSetFieldVisibilityEN._CurrTabName;
List<clsvSetFieldVisibilityEN> arrvSetFieldVisibilityObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
objDT.Columns.Add(convSetFieldVisibility.FuncModuleNameSim, Type.GetType("System.String"));
...
objDR[convSetFieldVisibility.FuncModuleNameSim] = objInFor[convSetFieldVisibility.FuncModuleNameSim];
