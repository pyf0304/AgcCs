...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxUserCodePathApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxUserCodePathEN objQxUserCodePathEN = clsQxUserCodePathBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxUserCodePathEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxUserCodePathApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxUserCodePathEN objQxUserCodePath_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxUserCodePathJSONObj = clsJSON.GetJsonFromObj(objQxUserCodePath_Cond);
dictParam.Add("strQxUserCodePathJSONObj", strQxUserCodePathJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxUserCodePathEN> arrQxUserCodePathObjLst;
 try
 {
if (objQxUserCodePath_Cond.sfFldComparisonOp == null)
{
objQxUserCodePath_Cond.dicFldComparisonOp = null;
}
else
{
objQxUserCodePath_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxUserCodePath_Cond.sfFldComparisonOp);
}
clsQxUserCodePathBL.SetUpdFlag(objQxUserCodePath_Cond);
 arrQxUserCodePathObjLst = clsQxUserCodePathBL.GetSubObjLstCache(objQxUserCodePath_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxUserCodePathEN objQxUserCodePath_Cond = new();
string strCondition = objQxUserCodePath_Cond
