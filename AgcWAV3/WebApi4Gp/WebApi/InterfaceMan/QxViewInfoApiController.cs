...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxViewInfoApi/GetObjByKey_Cache?ViewId=value
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
 /// </summary>
 /// <param name = "strViewId">表关键字</param>
 /// <returns>表对象</returns>
[HttpGet("GetObjByViewId_Cache")]
public ActionResult GetObjByViewIdCache(string strViewId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxViewInfoEN objQxViewInfoEN = clsQxViewInfoBL.GetObjByViewIdCache(strViewId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxViewInfoEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxViewInfoApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxViewInfoEN objQxViewInfo_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxViewInfoJSONObj = clsJSON.GetJsonFromObj(objQxViewInfo_Cond);
dictParam.Add("strQxViewInfoJSONObj", strQxViewInfoJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxViewInfoEN> arrQxViewInfoObjLst;
 try
 {
if (objQxViewInfo_Cond.sfFldComparisonOp == null)
{
objQxViewInfo_Cond.dicFldComparisonOp = null;
}
else
{
objQxViewInfo_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxViewInfo_Cond.sfFldComparisonOp);
}
clsQxViewInfoBL.SetUpdFlag(objQxViewInfo_Cond);
 arrQxViewInfoObjLst = clsQxViewInfoBL.GetSubObjLstCache(objQxViewInfo_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrViewIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByViewIdLst_Cache")]
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxViewInfoEN objQxViewInfo_Cond = new();
string strCondition = objQxViewInfo_Cond
