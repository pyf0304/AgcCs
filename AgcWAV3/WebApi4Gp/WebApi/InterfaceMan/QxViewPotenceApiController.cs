...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxViewPotenceApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxViewPotenceEN objQxViewPotenceEN = clsQxViewPotenceBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxViewPotenceEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxViewPotenceApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxViewPotenceEN objQxViewPotence_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxViewPotenceJSONObj = clsJSON.GetJsonFromObj(objQxViewPotence_Cond);
dictParam.Add("strQxViewPotenceJSONObj", strQxViewPotenceJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxViewPotenceEN> arrQxViewPotenceObjLst;
 try
 {
if (objQxViewPotence_Cond.sfFldComparisonOp == null)
{
objQxViewPotence_Cond.dicFldComparisonOp = null;
}
else
{
objQxViewPotence_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxViewPotence_Cond.sfFldComparisonOp);
}
clsQxViewPotenceBL.SetUpdFlag(objQxViewPotence_Cond);
 arrQxViewPotenceObjLst = clsQxViewPotenceBL.GetSubObjLstCache(objQxViewPotence_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxViewPotenceEN objQxViewPotence_Cond = new();
string strCondition = objQxViewPotence_Cond
