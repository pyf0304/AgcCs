...
namespace GeneralPlatform.WebApi
{
    ...
    public class QxPrjModuleApiController : ControllerBase
    {
        ...
        /// <summary>
        /// 获取当前关键字的记录对象,用对象的形式表示.
        /// 调用方法: GET /api/QxPrjModuleApi/GetObjByKey_Cache?ModuleId=value
        /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
        /// </summary>
        /// <param name = "strModuleId">表关键字</param>
        /// <returns>表对象</returns>
        [HttpGet("GetObjByModuleId_Cache")]
        public ActionResult GetObjByModuleIdCache(string strModuleId)
        {
            string strFunctionName = clsStackTrace.GetCurrFunction();
            Dictionary<string, string> dictParam = new()
            {
            ...
            try
            {
                clsQxPrjModuleEN objQxPrjModuleEN = clsQxPrjModuleBL.GetObjByModuleIdCache(strModuleId);
                return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxPrjModuleEN });
            }
            catch (Exception objException)
            {
        ...
        /// <summary>
        /// 根据条件分页获取JSON对象列表
        /// 调用方法: GET /api/QxPrjModuleApi/GetSubObjLst_Cache?objTabName_Cond=v1&
        /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
        /// </summary>
        /// <param name = "objPagerPara">分页获取记录的参数对象</param>
        /// <returns>返回JSON对象列表</returns>
        [HttpPost("GetSubObjLst_Cache")]
        public ActionResult GetSubObjLstCache([FromBody] clsQxPrjModuleEN objQxPrjModule_Cond)
        {
            string strFunctionName = clsStackTrace.GetCurrFunction();
            Dictionary<string, string> dictParam = new();
            string strQxPrjModuleJSONObj = clsJSON.GetJsonFromObj(objQxPrjModule_Cond);
            dictParam.Add("strQxPrjModuleJSONObj", strQxPrjModuleJSONObj);
            clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
            IEnumerable<clsQxPrjModuleEN> arrQxPrjModuleObjLst;
            try
            {
                if (objQxPrjModule_Cond.sfFldComparisonOp == null)
                {
                    objQxPrjModule_Cond.dicFldComparisonOp = null;
                }
                else
                {
                    objQxPrjModule_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxPrjModule_Cond.sfFldComparisonOp);
                }
                clsQxPrjModuleBL.SetUpdFlag(objQxPrjModule_Cond);
                arrQxPrjModuleObjLst = clsQxPrjModuleBL.GetSubObjLstCache(objQxPrjModule_Cond);
            }
            catch (Exception objException)
            {
                string strMsg = string.Format("{0}.(from {1})", objException.Message, clsStackTrace.GetCurrClassFunction());
        ...
        /// <param name = "arrModuleIdLst">所给的关键字</param>
        /// <returns>根据关键字获取的对象</returns>
        [HttpPost("GetObjLstByModuleIdLst_Cache")]
        ...
        public ActionResult AddNewRecord([FromBody] clsQxPrjModuleEN objQxPrjModuleEN)
        {
            ...
            try
            {
                clsQxPrjModuleEN objQxPrjModule_Cond = new();
                string strCondition = objQxPrjModule_Cond
