...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxFuncModule_AgcApi/GetObjByKey_Cache?FuncModuleAgcId=value
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
 /// </summary>
 /// <param name = "strFuncModuleAgcId">表关键字</param>
 /// <returns>表对象</returns>
[HttpGet("GetObjByFuncModuleAgcId_Cache")]
public ActionResult GetObjByFuncModuleAgcIdCache(string strFuncModuleAgcId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxFuncModule_AgcEN objQxFuncModule_AgcEN = clsQxFuncModule_AgcBL.GetObjByFuncModuleAgcIdCache(strFuncModuleAgcId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxFuncModule_AgcEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxFuncModule_AgcApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxFuncModule_AgcEN objQxFuncModule_Agc_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxFuncModule_AgcJSONObj = clsJSON.GetJsonFromObj(objQxFuncModule_Agc_Cond);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 try
 {
if (objQxFuncModule_Agc_Cond.sfFldComparisonOp == null)
{
objQxFuncModule_Agc_Cond.dicFldComparisonOp = null;
}
else
{
objQxFuncModule_Agc_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxFuncModule_Agc_Cond.sfFldComparisonOp);
}
clsQxFuncModule_AgcBL.SetUpdFlag(objQxFuncModule_Agc_Cond);
 arrQxFuncModule_AgcObjLst = clsQxFuncModule_AgcBL.GetSubObjLstCache(objQxFuncModule_Agc_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrFuncModuleAgcIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByFuncModuleAgcIdLst_Cache")]
