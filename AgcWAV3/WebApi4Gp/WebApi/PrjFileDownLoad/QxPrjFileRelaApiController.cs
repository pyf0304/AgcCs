...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxPrjFileRelaApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxPrjFileRelaEN objQxPrjFileRelaEN = clsQxPrjFileRelaBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxPrjFileRelaEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxPrjFileRelaApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxPrjFileRelaEN objQxPrjFileRela_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxPrjFileRelaJSONObj = clsJSON.GetJsonFromObj(objQxPrjFileRela_Cond);
dictParam.Add("strQxPrjFileRelaJSONObj", strQxPrjFileRelaJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxPrjFileRelaEN> arrQxPrjFileRelaObjLst;
 try
 {
if (objQxPrjFileRela_Cond.sfFldComparisonOp == null)
{
objQxPrjFileRela_Cond.dicFldComparisonOp = null;
}
else
{
objQxPrjFileRela_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxPrjFileRela_Cond.sfFldComparisonOp);
}
clsQxPrjFileRelaBL.SetUpdFlag(objQxPrjFileRela_Cond);
 arrQxPrjFileRelaObjLst = clsQxPrjFileRelaBL.GetSubObjLstCache(objQxPrjFileRela_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxPrjFileRelaEN objQxPrjFileRela_Cond = new();
string strCondition = objQxPrjFileRela_Cond
