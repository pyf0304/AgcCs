...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxUserMenusApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxUserMenusEN objQxUserMenusEN = clsQxUserMenusBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxUserMenusEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxUserMenusApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxUserMenusEN objQxUserMenus_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxUserMenusJSONObj = clsJSON.GetJsonFromObj(objQxUserMenus_Cond);
dictParam.Add("strQxUserMenusJSONObj", strQxUserMenusJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxUserMenusEN> arrQxUserMenusObjLst;
 try
 {
if (objQxUserMenus_Cond.sfFldComparisonOp == null)
{
objQxUserMenus_Cond.dicFldComparisonOp = null;
}
else
{
objQxUserMenus_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxUserMenus_Cond.sfFldComparisonOp);
}
clsQxUserMenusBL.SetUpdFlag(objQxUserMenus_Cond);
 arrQxUserMenusObjLst = clsQxUserMenusBL.GetSubObjLstCache(objQxUserMenus_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxUserMenusEN objQxUserMenus_Cond = new();
string strCondition = objQxUserMenus_Cond
