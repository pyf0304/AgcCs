...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxUserMenuDetailApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxUserMenuDetailEN objQxUserMenuDetailEN = clsQxUserMenuDetailBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxUserMenuDetailEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxUserMenuDetailApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxUserMenuDetailEN objQxUserMenuDetail_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxUserMenuDetailJSONObj = clsJSON.GetJsonFromObj(objQxUserMenuDetail_Cond);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 try
 {
if (objQxUserMenuDetail_Cond.sfFldComparisonOp == null)
{
objQxUserMenuDetail_Cond.dicFldComparisonOp = null;
}
else
{
objQxUserMenuDetail_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxUserMenuDetail_Cond.sfFldComparisonOp);
}
clsQxUserMenuDetailBL.SetUpdFlag(objQxUserMenuDetail_Cond);
 arrQxUserMenuDetailObjLst = clsQxUserMenuDetailBL.GetSubObjLstCache(objQxUserMenuDetail_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxUserMenuDetailEN objQxUserMenuDetail_Cond = new();
string strCondition = objQxUserMenuDetail_Cond
