...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxRoleMenusApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxRoleMenusEN objQxRoleMenusEN = clsQxRoleMenusBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxRoleMenusEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxRoleMenusApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxRoleMenusEN objQxRoleMenus_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxRoleMenusJSONObj = clsJSON.GetJsonFromObj(objQxRoleMenus_Cond);
dictParam.Add("strQxRoleMenusJSONObj", strQxRoleMenusJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxRoleMenusEN> arrQxRoleMenusObjLst;
 try
 {
if (objQxRoleMenus_Cond.sfFldComparisonOp == null)
{
objQxRoleMenus_Cond.dicFldComparisonOp = null;
}
else
{
objQxRoleMenus_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxRoleMenus_Cond.sfFldComparisonOp);
}
clsQxRoleMenusBL.SetUpdFlag(objQxRoleMenus_Cond);
 arrQxRoleMenusObjLst = clsQxRoleMenusBL.GetSubObjLstCache(objQxRoleMenus_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxRoleMenusEN objQxRoleMenus_Cond = new();
string strCondition = objQxRoleMenus_Cond
