...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxPrjMenusApi/GetObjByKey_Cache?MenuId=value
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
 /// </summary>
 /// <param name = "strMenuId">表关键字</param>
 /// <param name = "strQxPrjId">缓存的分类字段</param>
 /// <returns>表对象</returns>
[HttpGet("GetObjByMenuId_Cache")]
public ActionResult GetObjByMenuIdCache(string strMenuId, string strQxPrjId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxPrjMenusEN objQxPrjMenusEN = clsQxPrjMenusBL.GetObjByMenuIdCache(strMenuId, strQxPrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxPrjMenusEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxPrjMenusApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxPrjMenusEN objQxPrjMenus_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxPrjMenusJSONObj = clsJSON.GetJsonFromObj(objQxPrjMenus_Cond);
dictParam.Add("strQxPrjMenusJSONObj", strQxPrjMenusJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxPrjMenusEN> arrQxPrjMenusObjLst;
 try
 {
if (objQxPrjMenus_Cond.sfFldComparisonOp == null)
{
objQxPrjMenus_Cond.dicFldComparisonOp = null;
}
else
{
objQxPrjMenus_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxPrjMenus_Cond.sfFldComparisonOp);
}
clsQxPrjMenusBL.SetUpdFlag(objQxPrjMenus_Cond);
 arrQxPrjMenusObjLst = clsQxPrjMenusBL.GetSubObjLstCache(objQxPrjMenus_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrMenuIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByMenuIdLst_Cache")]
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxPrjMenusEN objQxPrjMenus_Cond = new();
string strCondition = objQxPrjMenus_Cond
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 // 表:QxPrjMenus的主键是前缀自增，但该表中没有一个相关的前缀字段。请检查！(In AutoGCLib.WA_Srv4CSharp:Gen_AddNewRecordWithMaxId)
