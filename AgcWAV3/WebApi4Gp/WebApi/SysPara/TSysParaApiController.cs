...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/TSysParaApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsTSysParaEN objTSysParaEN = clsTSysParaBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objTSysParaEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/TSysParaApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsTSysParaEN objTSysPara_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strTSysParaJSONObj = clsJSON.GetJsonFromObj(objTSysPara_Cond);
dictParam.Add("strTSysParaJSONObj", strTSysParaJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsTSysParaEN> arrTSysParaObjLst;
 try
 {
if (objTSysPara_Cond.sfFldComparisonOp == null)
{
objTSysPara_Cond.dicFldComparisonOp = null;
}
else
{
objTSysPara_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objTSysPara_Cond.sfFldComparisonOp);
}
clsTSysParaBL.SetUpdFlag(objTSysPara_Cond);
 arrTSysParaObjLst = clsTSysParaBL.GetSubObjLstCache(objTSysPara_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
