...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxPotenceTypeApi/GetObjByKey_Cache?PotenceTypeId=value
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
 /// </summary>
 /// <param name = "strPotenceTypeId">表关键字</param>
 /// <returns>表对象</returns>
[HttpGet("GetObjByPotenceTypeId_Cache")]
public ActionResult GetObjByPotenceTypeIdCache(string strPotenceTypeId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxPotenceTypeEN objQxPotenceTypeEN = clsQxPotenceTypeBL.GetObjByPotenceTypeIdCache(strPotenceTypeId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxPotenceTypeEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxPotenceTypeApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxPotenceTypeEN objQxPotenceType_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxPotenceTypeJSONObj = clsJSON.GetJsonFromObj(objQxPotenceType_Cond);
dictParam.Add("strQxPotenceTypeJSONObj", strQxPotenceTypeJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxPotenceTypeEN> arrQxPotenceTypeObjLst;
 try
 {
if (objQxPotenceType_Cond.sfFldComparisonOp == null)
{
objQxPotenceType_Cond.dicFldComparisonOp = null;
}
else
{
objQxPotenceType_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxPotenceType_Cond.sfFldComparisonOp);
}
clsQxPotenceTypeBL.SetUpdFlag(objQxPotenceType_Cond);
 arrQxPotenceTypeObjLst = clsQxPotenceTypeBL.GetSubObjLstCache(objQxPotenceType_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrPotenceTypeIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByPotenceTypeIdLst_Cache")]
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxPotenceTypeEN objQxPotenceType_Cond = new();
string strCondition = objQxPotenceType_Cond
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 // 表:QxPotenceType的主键是前缀自增，但该表中没有一个相关的前缀字段。请检查！(In AutoGCLib.WA_Srv4CSharp:Gen_AddNewRecordWithMaxId)
