...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxRolePotenceV2Api/GetObjByKey_Cache?RId=value
...
public ActionResult GetObjByKeyLstCache(int intRId,int intPId, string strQxPrjId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxRolePotenceV2EN objQxRolePotenceV2EN = clsQxRolePotenceV2BL.GetObjByKeyLstCache(intRId,intPId, strQxPrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxRolePotenceV2EN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxRolePotenceV2Api/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxRolePotenceV2EN objQxRolePotenceV2_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxRolePotenceV2JSONObj = clsJSON.GetJsonFromObj(objQxRolePotenceV2_Cond);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 try
 {
if (objQxRolePotenceV2_Cond.sfFldComparisonOp == null)
{
objQxRolePotenceV2_Cond.dicFldComparisonOp = null;
}
else
{
objQxRolePotenceV2_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxRolePotenceV2_Cond.sfFldComparisonOp);
}
clsQxRolePotenceV2BL.SetUpdFlag(objQxRolePotenceV2_Cond);
 arrQxRolePotenceV2ObjLst = clsQxRolePotenceV2BL.GetSubObjLstCache(objQxRolePotenceV2_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
