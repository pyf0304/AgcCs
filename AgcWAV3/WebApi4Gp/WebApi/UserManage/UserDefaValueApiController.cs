...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/UserDefaValueApi/GetObjByKey_Cache?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsUserDefaValueEN objUserDefaValueEN = clsUserDefaValueBL.GetObjBymIdCache(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objUserDefaValueEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/UserDefaValueApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsUserDefaValueEN objUserDefaValue_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strUserDefaValueJSONObj = clsJSON.GetJsonFromObj(objUserDefaValue_Cond);
dictParam.Add("strUserDefaValueJSONObj", strUserDefaValueJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsUserDefaValueEN> arrUserDefaValueObjLst;
 try
 {
if (objUserDefaValue_Cond.sfFldComparisonOp == null)
{
objUserDefaValue_Cond.dicFldComparisonOp = null;
}
else
{
objUserDefaValue_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objUserDefaValue_Cond.sfFldComparisonOp);
}
clsUserDefaValueBL.SetUpdFlag(objUserDefaValue_Cond);
 arrUserDefaValueObjLst = clsUserDefaValueBL.GetSubObjLstCache(objUserDefaValue_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsUserDefaValueEN objUserDefaValue_Cond = new();
string strCondition = objUserDefaValue_Cond
