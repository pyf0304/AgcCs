...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxUserStateApi/GetObjByKey_Cache?UserStateId=value
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
 /// </summary>
 /// <param name = "strUserStateId">表关键字</param>
 /// <returns>表对象</returns>
[HttpGet("GetObjByUserStateId_Cache")]
public ActionResult GetObjByUserStateIdCache(string strUserStateId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxUserStateEN objQxUserStateEN = clsQxUserStateBL.GetObjByUserStateIdCache(strUserStateId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxUserStateEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxUserStateApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxUserStateEN objQxUserState_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxUserStateJSONObj = clsJSON.GetJsonFromObj(objQxUserState_Cond);
dictParam.Add("strQxUserStateJSONObj", strQxUserStateJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsQxUserStateEN> arrQxUserStateObjLst;
 try
 {
if (objQxUserState_Cond.sfFldComparisonOp == null)
{
objQxUserState_Cond.dicFldComparisonOp = null;
}
else
{
objQxUserState_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxUserState_Cond.sfFldComparisonOp);
}
clsQxUserStateBL.SetUpdFlag(objQxUserState_Cond);
 arrQxUserStateObjLst = clsQxUserStateBL.GetSubObjLstCache(objQxUserState_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrUserStateIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByUserStateIdLst_Cache")]
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxUserStateEN objQxUserState_Cond = new();
string strCondition = objQxUserState_Cond
