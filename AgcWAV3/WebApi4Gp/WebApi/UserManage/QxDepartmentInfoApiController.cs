...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/QxDepartmentInfoApi/GetObjByKey_Cache?DepartmentId=value
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetObjByKeyId_Cache)
 /// </summary>
 /// <param name = "strDepartmentId">表关键字</param>
 /// <returns>表对象</returns>
[HttpGet("GetObjByDepartmentId_Cache")]
public ActionResult GetObjByDepartmentIdCache(string strDepartmentId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxDepartmentInfoEN objQxDepartmentInfoEN = clsQxDepartmentInfoBL.GetObjByDepartmentIdCache(strDepartmentId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objQxDepartmentInfoEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxDepartmentInfoApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsQxDepartmentInfoEN objQxDepartmentInfo_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxDepartmentInfoJSONObj = clsJSON.GetJsonFromObj(objQxDepartmentInfo_Cond);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 try
 {
if (objQxDepartmentInfo_Cond.sfFldComparisonOp == null)
{
objQxDepartmentInfo_Cond.dicFldComparisonOp = null;
}
else
{
objQxDepartmentInfo_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objQxDepartmentInfo_Cond.sfFldComparisonOp);
}
clsQxDepartmentInfoBL.SetUpdFlag(objQxDepartmentInfo_Cond);
 arrQxDepartmentInfoObjLst = clsQxDepartmentInfoBL.GetSubObjLstCache(objQxDepartmentInfo_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrDepartmentIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByDepartmentIdLst_Cache")]
