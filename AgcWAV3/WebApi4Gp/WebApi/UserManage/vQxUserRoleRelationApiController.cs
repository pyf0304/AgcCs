...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vQxUserRoleRelationApi/GetObjByKey_Cache?mId=value
...
public ActionResult GetObjBymIdCache(long lngmId, string strQxPrjId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsvQxUserRoleRelationEN objvQxUserRoleRelationEN = clsvQxUserRoleRelationBL.GetObjBymIdCache(lngmId, strQxPrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvQxUserRoleRelationEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vQxUserRoleRelationApi/GetSubObjLst_Cache?objTabName_Cond=v1&
 /// (AutoGCLib.WA_Srv4CSharp:Gen_GetSubObjLst_Cache)
 /// </summary>
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
 [HttpPost("GetSubObjLst_Cache")]
 public ActionResult GetSubObjLstCache([FromBody]clsvQxUserRoleRelationEN objvQxUserRoleRelation_Cond)
 {
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strvQxUserRoleRelationJSONObj = clsJSON.GetJsonFromObj(objvQxUserRoleRelation_Cond);
dictParam.Add("strvQxUserRoleRelationJSONObj", strvQxUserRoleRelationJSONObj);
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 IEnumerable<clsvQxUserRoleRelationEN> arrvQxUserRoleRelationObjLst;
 try
 {
if (objvQxUserRoleRelation_Cond.sfFldComparisonOp == null)
{
objvQxUserRoleRelation_Cond.dicFldComparisonOp = null;
}
else
{
objvQxUserRoleRelation_Cond.dicFldComparisonOp = JsonConvert.DeserializeObject<Dictionary<string, string>>(objvQxUserRoleRelation_Cond.sfFldComparisonOp);
}
clsvQxUserRoleRelationBL.SetUpdFlag(objvQxUserRoleRelation_Cond);
 arrvQxUserRoleRelationObjLst = clsvQxUserRoleRelationBL.GetSubObjLstCache(objvQxUserRoleRelation_Cond);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
