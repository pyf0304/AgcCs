...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/TabStateApi/GetMaxStrId?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
string strMaxstrTabStateId = clsTabStateBL.GetMaxStrId_S();
return Ok(new { errorId = 0, errorMsg = "", returnStr =  strMaxstrTabStateId });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsTabStateEN objTabStateCond = new();
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 通过JSON对象来添加记录对象
 /// 调用方法: POST /api/TabStateApi/AddNewRecordWithMaxId
 /// 在Body区传输objTabStateEN的JSON对象
...
public ActionResult AddNewRecordWithMaxId([FromBody]clsTabStateEN objTabStateEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strTabStateJSONObj = clsJSON.GetJsonFromObj(objTabStateEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 {
    clsTabStateBL.AccessFldValueNull(objTabStateEN);
string strTabStateId = objTabStateEN.AddNewRecordWithMaxId();
return Ok(new { errorId = 0, errorMsg = "", returnStr =  strTabStateId });
 }
 catch (Exception objException)
 {
