...
public class  DataNodeApiController : ControllerBase
{ 
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsDataNodeEN> arrDataNodeObjLst = clsDataNodeBL.GetObjLstCache(strPrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrDataNodeObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstDataNodeId = lstDataNodeId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsDataNodeEN> arrDataNodeObjLst = clsDataNodeBL.GetObjLstByDataNodeIdLstCache(lstDataNodeId, PrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrDataNodeObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 lngDataNodeId = clsDataNodeBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 List<clsDataNodeEN> arrDataNodeObjLst;
 try
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intRecCount = clsDataNodeBL.SetFldValue(clsDataNodeEN._CurrTabName, strFldName, strValue, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsDataNodeBL.IsExist(lngDataNodeId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsDataNodeEN objDataNodeCond = new();
...
return Ok(new { errorId = 1, errorMsg = strMsg });
    ...
    clsDataNodeBL.AccessFldValueNull(objDataNodeEN);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: POST /api/DataNodeApi/AddNewRecordWithReturnKey
 /// 在Body区传输objDataNodeEN的JSON对象
...
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
dictParam.Add("objDataNodeEN", JsonConvert.SerializeObject(objDataNodeEN));
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 }
try
{
...
{
long lngDataNodeId = Id;
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
["Id"] = lngDataNodeId.ToString()
 };
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 if (lngDataNodeId <= 0)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intRecNum = clsDataNodeBL.DelRecord(lngDataNodeId);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intRecNum = clsDataNodeBL.DelDataNodes(arrKey);
return Ok(new { errorId = 0, errorMsg = "", returnInt = intRecNum });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <returns>返回删除的记录数。</returns>
[HttpGet("DelDataNodesByCond")]
public ActionResult DelDataNodesByCond(string strWhereCond)
{
strWhereCond = strWhereCond.Replace("'% ", "'%");
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
