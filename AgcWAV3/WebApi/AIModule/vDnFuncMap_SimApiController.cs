...
{
 /// <summary>
 /// vDnFuncMap_SimController 的摘要说明
 /// (AutoGCLib.WA_Srv4CSharp:GeneCode)
 /// </summary>
[ApiController]
[Route("[controller]")]
[ApiExplorerSettings(IgnoreApi = true)]
public class  vDnFuncMap_SimApiController : ControllerBase
{ 

 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetObjByDnFuncMapId?DnFuncMapId=value
...
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvDnFuncMap_SimEN });
 }
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetObjLst?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<clsvDnFuncMap_SimEN> arrvDnFuncMap_SimObjLst = clsvDnFuncMap_SimBL.GetObjLst(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvDnFuncMap_SimObjLst });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvDnFuncMap_SimEN> arrvDnFuncMap_SimObjLst = clsvDnFuncMap_SimBL.GetObjLstCache(strPrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvDnFuncMap_SimObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/vDnFuncMap_Simapi/GetObjLstByDnFuncMapIdLst
...
List<string> lstDnFuncMapId = new(arrDnFuncMapId);
 try
 {
List<clsvDnFuncMap_SimEN> arrvDnFuncMap_SimObjLst = clsvDnFuncMap_SimBL.GetObjLstByDnFuncMapIdLst(lstDnFuncMapId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvDnFuncMap_SimObjLst });
 }
 catch (Exception objException)
 {
...
List<string> lstDnFuncMapId = new(arrDnFuncMapId);
 try
 {
IEnumerable<clsvDnFuncMap_SimEN> arrvDnFuncMap_SimObjLst = clsvDnFuncMap_SimBL.GetObjLstByDnFuncMapIdLstCache(lstDnFuncMapId, PrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvDnFuncMap_SimObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取顶部对象列表
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetTopObjLst?intTopSize=v1&strOrderBy=v2&strWhereCond=v3&
...
clsPubFun_WebApi.AccessException(strMsg, HttpStatusCode.NotFound);
 ...
 try
 {
List<clsvDnFuncMap_SimEN> arrvDnFuncMap_SimObjLst = clsvDnFuncMap_SimBL.GetTopObjLst(objTopPara);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvDnFuncMap_SimObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetFirstID?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 strDnFuncMapId = clsvDnFuncMap_SimBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前表满足条件的第一条记录的对象
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetFirstObj?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 clsvDnFuncMap_SimEN objvDnFuncMap_SimEN = clsvDnFuncMap_SimBL.GetFirstObj_S(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvDnFuncMap_SimEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetObjLstByPager?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 List<clsvDnFuncMap_SimEN> arrvDnFuncMap_SimObjLst;
 try
 {
 arrvDnFuncMap_SimObjLst = clsvDnFuncMap_SimBL.GetObjLstByPager(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvDnFuncMap_SimObjLst });
 ...
 /// <summary>
 /// 功能:获取某一条件的记录数
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetRecCountByCond?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intCount = clsvDnFuncMap_SimBL.GetRecCountByCond(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vDnFuncMap_SimApi/GetFldValue?strWhereCond=v1&strFldName=v2&strTabName=v3&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<string> arrList = clsvDnFuncMap_SimBL.GetFldValue(strFldName, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 功能:判断是否存在某一条件的记录
 /// 调用方法: GET /api/vDnFuncMap_SimApi/IsExistRecord?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvDnFuncMap_SimBL.IsExistRecord(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/vDnFuncMap_SimApi/IsExist?DnFuncMapId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvDnFuncMap_SimBL.IsExist(strDnFuncMapId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
