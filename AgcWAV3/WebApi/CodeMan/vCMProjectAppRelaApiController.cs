 ...
 /*-- -- -- -- -- -- -- -- -- -- --
 类名:vCmProjectAppRelaApiController
 表名:vCmProjectAppRela(00050634)
 * 版本:2024.10.15.1(服务器:WIN-SRV103-116)
 日期:2024/10/16 09:50:17
 生成者:pyf
 生成服务器IP:
 工程名称:AGC(0005)
...
{
 /// <summary>
 /// vCmProjectAppRelaController 的摘要说明
 /// (AutoGCLib.WA_Srv4CSharp:GeneCode)
 /// </summary>
[ApiController]
[Route("[controller]")]
[ApiExplorerSettings(IgnoreApi = true)]
public class  vCmProjectAppRelaApiController : ControllerBase
{ 

 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetObjByCMProjectAppRelaId?CMProjectAppRelaId=value
...
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvCmProjectAppRelaEN });
 }
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetObjLst?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<clsvCmProjectAppRelaEN> arrvCmProjectAppRelaObjLst = clsvCmProjectAppRelaBL.GetObjLst(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvCmProjectAppRelaObjLst });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvCmProjectAppRelaEN> arrvCmProjectAppRelaObjLst = clsvCmProjectAppRelaBL.GetObjLstCache(strPrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvCmProjectAppRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/vCmProjectAppRelaapi/GetObjLstByCMProjectAppRelaIdLst
...
List<long> lstCMProjectAppRelaId = lstCMProjectAppRelaId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
List<clsvCmProjectAppRelaEN> arrvCmProjectAppRelaObjLst = clsvCmProjectAppRelaBL.GetObjLstByCMProjectAppRelaIdLst(lstCMProjectAppRelaId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvCmProjectAppRelaObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstCMProjectAppRelaId = lstCMProjectAppRelaId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsvCmProjectAppRelaEN> arrvCmProjectAppRelaObjLst = clsvCmProjectAppRelaBL.GetObjLstByCMProjectAppRelaIdLstCache(lstCMProjectAppRelaId, PrjId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvCmProjectAppRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取顶部对象列表
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetTopObjLst?intTopSize=v1&strOrderBy=v2&strWhereCond=v3&
...
clsPubFun_WebApi.AccessException(strMsg, HttpStatusCode.NotFound);
 ...
 try
 {
List<clsvCmProjectAppRelaEN> arrvCmProjectAppRelaObjLst = clsvCmProjectAppRelaBL.GetTopObjLst(objTopPara);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvCmProjectAppRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetFirstID?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 lngCMProjectAppRelaId = clsvCmProjectAppRelaBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前表满足条件的第一条记录的对象
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetFirstObj?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 clsvCmProjectAppRelaEN objvCmProjectAppRelaEN = clsvCmProjectAppRelaBL.GetFirstObj_S(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvCmProjectAppRelaEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetObjLstByPager?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 List<clsvCmProjectAppRelaEN> arrvCmProjectAppRelaObjLst;
 try
 {
 arrvCmProjectAppRelaObjLst = clsvCmProjectAppRelaBL.GetObjLstByPager(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvCmProjectAppRelaObjLst });
 ...
 /// <summary>
 /// 功能:获取某一条件的记录数
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetRecCountByCond?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intCount = clsvCmProjectAppRelaBL.GetRecCountByCond(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vCmProjectAppRelaApi/GetFldValue?strWhereCond=v1&strFldName=v2&strTabName=v3&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<string> arrList = clsvCmProjectAppRelaBL.GetFldValue(strFldName, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 功能:判断是否存在某一条件的记录
 /// 调用方法: GET /api/vCmProjectAppRelaApi/IsExistRecord?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvCmProjectAppRelaBL.IsExistRecord(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/vCmProjectAppRelaApi/IsExist?CMProjectAppRelaId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvCmProjectAppRelaBL.IsExist(lngCMProjectAppRelaId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
