...
namespace AGC
{
    ...
    /// </summary>
    public partial class frmUserCodePrjMainPath_QD_LV : System.Windows.Forms.Form, IBindListView
    {
        ...
        public frmUserCodePrjMainPath_QD_LV()
        {
            ...
            dgvUserCodePrjMainPath.DefaultCellStyle.WrapMode = DataGridViewTriState.True;
        }
        private void frmUserCodePrjMainPath_QD_Load(object sender, System.EventArgs e)
        {
            ...
            this.Text = string.Format("{0}-代码生成主路径设置－列表", clsPubVar.objCurrSelProjectEN.PrjName);
            this.SetAllComboBox();
        ...
        {
            if (chkDIskAllCmPrjApp.Checked == true)
            {
                this.btnDelRec.Enabled = false;
                return;
            }
            if (this.dgvUserCodePrjMainPath.SelectedRows.Count >= 1)
            {
        ...
        //            clsUserDefaValue_LocalExWApi.setUserDefaValue(clsPubVar.CurrPrjId, clsPubVar.CurrSelPrjId, clsSysParaEN.strUserId, "默认CM工程", strCmPrjId);
        //        }
        //        clsUserCodePrjMainPathExWApi.CopyUserCodePrjMainPathFromTemplate(clsSysParaEN.CurrSelPrjId, strCmPrjId, strMachineName, clsSysParaEN.strUserId);
        //    }
        ...
        //    {
        //        StringBuilder sbCodePath = new StringBuilder();
        //        StringBuilder sbLogPath = new StringBuilder();
        //        string strCodePathBackup = "";
        //        var arrUserCodePrjMainPath_MachineName = clsUserCodePrjMainPath_MachineNameExWApi.GetObjLstByUserCodePrjMainPathIdCache(objvUserCodePrjMainPathEN.UserCodePrjMainPathId, 
        //            objvUserCodePrjMainPathEN.PrjId);
        //        foreach (var objUserCodePrjMainPath_MachineName in arrUserCodePrjMainPath_MachineName)
        //        {
        //            if (objUserCodePrjMainPath_MachineName != null)
        //            {
        //                sbCodePath.AppendLine(objUserCodePrjMainPath_MachineName.CodePath+"\n");
        //                sbLogPath.AppendLine(objUserCodePrjMainPath_MachineName.LogPath + "\n");
        ...
        //        lvivUserCodePrjMainPath.SubItems.Add(sbCodePath.ToString());
        ...
        //        lvivUserCodePrjMainPath.SubItems.Add(sbLogPath.ToString());
        ...
        //}
        private void BindDgv_vUserCodePrjMainPath()
        {
            //操作步骤：(共4步)
            //	1、组合界面条件串；
            //	2、根据条件串获取该表满足条件的DataTable；
            ...
            ListViewItem lvivUserCodePrjMainPath;
            List<clsvUserCodePrjMainPathEN> arrvUserCodePrjMainPath_Rela;
            //	1、组合界面条件串；
            string strWhereCond = this.CombineUserCodePrjMainPathCondion(false);	//  CombineUserCodePrjMainPathCondion();
            //strWhereCond += string.Format(" Order By {0}", clsvUserCodePrjMainPathEN.con_OrderNum);
            //	2、根据条件串获取该表满足条件的DataTable；
          
            if (chkDIskAllCmPrjApp.Checked)
            {
                var arrvCmProjectAppRela = clsvCmProjectAppRelaWApi.GetObjLstCache(clsPubVar.CurrSelPrjId);
                arrvUserCodePrjMainPath_Rela = arrvCmProjectAppRela.Select(clsvCmProjectAppRelaExWApi.CopyTovUserCodePrjMainPath).ToList();
                List<clsvUserCodePrjMainPathEN> arrvUserCodePrjMainPathObjList = clsvUserCodePrjMainPathWApi.GetObjLst(strWhereCond);

                foreach (var objInFor in arrvUserCodePrjMainPath_Rela)
                {
                    var obj = arrvUserCodePrjMainPathObjList.Find(x=>x.CmPrjId == objInFor.CmPrjId && x.ApplicationTypeId == objInFor.ApplicationTypeId);
                    if (obj == null)
                    {
                        objInFor.InUse = false;
                        objInFor.IsTemplate = false;
                    }
                    else
                    {
                        objInFor.InUse = obj.InUse;
                        objInFor.UpdDate = obj.UpdDate;
                        objInFor.IsTemplate = obj.IsTemplate;
                        objInFor.UserCodePrjMainPathId = obj.UserCodePrjMainPathId;
                    }
                }
            }
            else
            {
                arrvUserCodePrjMainPath_Rela = clsvUserCodePrjMainPathWApi.GetObjLst(strWhereCond);
            }
            if (arrvUserCodePrjMainPath_Rela.Count == 0)
            {
                string strCmPrjId = clsUserDefaValue_LocalExWApi.getUserDefaValue(clsPubVar.CurrPrjId, clsPubVar.CurrSelPrjId, clsSysParaEN.strUserId, "默认CM工程");
                if (string.IsNullOrEmpty(strCmPrjId) == true)
                {
            ...
            var arrvUserCodePrjMainPathExObjList = arrvUserCodePrjMainPath_Rela.Select(clsvUserCodePrjMainPathExWApi.CopyToEx).ToList();
            //	3、设置ListView的列头信息
            ...
            bool bolIsExistCodePath = false;
            bool bolIsExistCodePathBackup = false;
            foreach (clsvUserCodePrjMainPathENEx objInFor in arrvUserCodePrjMainPathExObjList)
            {
                StringBuilder sbCodePath = new StringBuilder();
                StringBuilder sbIsExistCodePath = new StringBuilder();
                StringBuilder sbIsExistCodePathBackup = new StringBuilder();
                StringBuilder sbLogPath = new StringBuilder();
                string strCodePathBackup = "";
                var arrUserCodePrjMainPath_MachineName = clsUserCodePrjMainPath_MachineNameExWApi.GetObjLstByUserCodePrjMainPathIdCache(objInFor.UserCodePrjMainPathId,
                    objInFor.PrjId);
                foreach (var objUserCodePrjMainPath_MachineName in arrUserCodePrjMainPath_MachineName)
                {
                    if (objUserCodePrjMainPath_MachineName != null)
                    {
                        sbCodePath.AppendLine($"{objUserCodePrjMainPath_MachineName.MachineName}:{objUserCodePrjMainPath_MachineName.CodePath} \n");
                        sbLogPath.AppendLine(objUserCodePrjMainPath_MachineName.LogPath + "\n");
                        bolIsExistCodePath = Directory.Exists(objUserCodePrjMainPath_MachineName.CodePath);
                        string strIsExistCodePath = bolIsExistCodePath ? "是" : "否";
                        sbIsExistCodePath.AppendLine($"{strIsExistCodePath}\n");
                        bolIsExistCodePathBackup = Directory.Exists(objUserCodePrjMainPath_MachineName.CodePathBackup);
                        string strIsExistCodePathBackup = bolIsExistCodePath ? "是" : "否";
                        sbIsExistCodePathBackup.AppendLine($"{strIsExistCodePathBackup}\n");
                    }
                }
                objInFor.CodePathEx = sbCodePath.ToString();
                objInFor.IsExistCodePathEx = sbIsExistCodePath.ToString();
                objInFor.LogPathEx = sbLogPath.ToString();
                
                objInFor. InUseEx = objInFor.InUse ? "是" : "否";
                //objInFor.CodePath = "aabb";// sbCodePath.ToString();

                //objInFor.ApplicationTypeSimName = "aa\nbb\ncc\ndd";
                lvivUserCodePrjMainPath = new ListViewItem();
                lvivUserCodePrjMainPath.Tag = objInFor.UserCodePrjMainPathId;
                ...
                if (clsPubVar.CurrSelPrjId == "0000")
                {
                    lvivUserCodePrjMainPath.SubItems.Add(objInFor.IsTemplate ? "是模板" : "否");
                }
                ...
                if (chkDIskBackup.Checked == true)
                {
                    lvivUserCodePrjMainPath = new ListViewItem();
                    lvivUserCodePrjMainPath.Tag = objInFor.UserCodePrjMainPathId;
                    lvivUserCodePrjMainPath.Text = objInFor.UserId;
                    //lvivUserCodePrjMainPath.SubItems.Add(objvUserCodePrjMainPathEN.UserId);
                    
                    lvivUserCodePrjMainPath.SubItems.Add(objInFor.AppOrderNum.ToString());
            ...
            this.dgvUserCodePrjMainPath.AutoGenerateColumns = false;
            this.dgvUserCodePrjMainPath.DataSource = arrvUserCodePrjMainPathExObjList;
            //if (bolIsHaveUpdate == true) BindLv_vUserCodePrjMainPath();
            //	4、设置记录数的状态，
            //		在本界面中是把状态显示在控件txtRecCount中。
            this.lblRecNum.Text = "记录数：" + arrvUserCodePrjMainPathExObjList.Count.ToString();
        }
        ...
        private string GetCurrKeyBak(DataGrid objDataGrid)
        {
        ...
        private string GetCurrKey(DataGridView objDataGridView)
        {
            if (objDataGridView.CurrentRow == null) return "";
            string strUserCodePrjMainPathId = objDataGridView.CurrentRow.Cells["dgvcUserCodePrjMainPathId"].Value.ToString();
            //if (strmId == "") return 0;
            //long lngmId = long.Parse(strmId);
            return strUserCodePrjMainPathId;
        }
        private string GetCurr_CMProjectAppRelaId(DataGridView objDataGridView)
        {
            if (objDataGridView.CurrentRow == null) return "";
            string strUserCodePrjMainPathId = objDataGridView.CurrentRow.Cells["dgvcCMProjectAppRelaId"].Value.ToString();
            //if (strmId == "") return 0;
            //long lngmId = long.Parse(strmId);
            return strUserCodePrjMainPathId;
        }
        ...
        /// <param name = "objDataGrid">所单击的DataGrid</param>
        /// <returns>当前行的主键值</returns>
        private string GetCurrKey(DataGridViewRow objDataGridViewRow)
        {
            if (objDataGridViewRow == null) return "";
            string strmId = objDataGridViewRow.Cells["dgvcUserCodePrjMainPathId"].Value.ToString();
            //long lngmId = long.Parse(strmId);
            return strmId;
        }
        private string GetCurr_CMProjectAppRelaId(DataGridViewRow objDataGridViewRow)
        {
            if (objDataGridViewRow == null) return "";
            string strmId = objDataGridViewRow.Cells["dgvcCMProjectAppRelaId"].Value.ToString();
            //long lngmId = long.Parse(strmId);
            return strmId;
        }
        /// <summary>
        ...
        private void UpdateUserCodePrjMainPathRecord()
        {
            ...
            string lngmId = "";
            if (this.dgvUserCodePrjMainPath.SelectedRows.Count > 0)
            {
                //修改当前选择行的第1行
                lngmId = GetCurrKey(this.dgvUserCodePrjMainPath.SelectedRows[0]);
            }
            else
            {
                //修改当前行
                lngmId = GetCurrKey(this.dgvUserCodePrjMainPath);
            }
            frmUserCodePrjMainPath_UI frm = new frmUserCodePrjMainPath_UI();
            string strKeyId = lngmId;
        ...
        private void DeleteUserCodePrjMainPathRecord()
        {
            ...
            //    return;
            //}
            var strPrjId = clsPubVar.CurrSelPrjId;
            DialogResult result;
            result = MessageBox.Show("你真的要删除该记录吗？", "确认删除", MessageBoxButtons.YesNo);
            if (result == DialogResult.Yes)
            {
                if (dgvUserCodePrjMainPath.SelectedRows.Count > 0)
                {
                    foreach (DataGridViewRow objDGVR in this.dgvUserCodePrjMainPath.SelectedRows)
                    {
                        string strUserCodePrjMainPathId =  GetCurrKey(objDGVR);
                        try
                        {
                            if (clsUserCodePrjMainPathExWApi.DelRecordEx(strUserCodePrjMainPathId, strPrjId) == false)
                            {
        ...
        private void btnCopyRec_Click(object sender, EventArgs e)
        {
            ...
            string strKeyId = GetCurrKey(dgvUserCodePrjMainPath);
            lblMsg.Text = "";
        ...
        private void dgvUserCodePrjMainPath_Click(object sender, System.EventArgs e)
        {
            this.JudgebtnDelRec(); //用来判断删除按钮是否可用
            this.JudgebtnUpdate(); //用来判断修改按钮是否可用
        }
        private void dgvUserCodePrjMainPath_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            this.JudgebtnDelRec(); //用来判断删除按钮是否可用
            this.JudgebtnUpdate(); //用来判断修改按钮是否可用
        }

        private void dgvUserCodePrjMainPath_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    //修改当前选择行的第1行
                    lngmId = GetCurr_CMProjectAppRelaId(this.dgvUserCodePrjMainPath.SelectedRows[0]);
                }
                else
                {
                    //修改当前行
                    lngmId = GetCurr_CMProjectAppRelaId(this.dgvUserCodePrjMainPath);
                }
                //Point p1 = new Point();
                var relativeMousePoint = dgvUserCodePrjMainPath.PointToClient(Cursor.Position);
                contextMenuStrip1.Show(dgvUserCodePrjMainPath, relativeMousePoint);
                //MessageBox.Show($"右击:{lngmId}" );
        ...
        private void 使用ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                //修改当前选择行的第1行
                lngCMProjectAppRelaId = long.Parse( GetCurr_CMProjectAppRelaId(this.dgvUserCodePrjMainPath.SelectedRows[0]));
            }
            else
            {
                //修改当前行
                lngCMProjectAppRelaId = long.Parse( GetCurr_CMProjectAppRelaId(this.dgvUserCodePrjMainPath));
            }
            //MessageBox.Show($"使用:{lngmId}");
            ...
            try
            {
                ...
                int intCount = clsUserCodePrjMainPathExWApi.CreateUserCodePrjMainPathByCmProjectApp(lngCMProjectAppRelaId, clsPubVar.CurrSelPrjId, strMachineName, clsSysParaEN.strUserId);
                BindDgv_vUserCodePrjMainPath();
                string strMsg = string.Format("复制模板完成！共复制了：[{0}]条记录！", intCount);
                MessageBox.Show(strMsg);
